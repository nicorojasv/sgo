...
class Requerimiento_asc_trabajadores_model extends CI_Model{
	...
	function get_requerimiento_usu($id_usuario){
		$this->db->select('rasc.referido, rasc.status, ar.nombre nombre_area, car.nombre nombre_cargo, req.id as codigo_requerimiento, req.nombre nombre_req, req.f_solicitud, ep.nombre empresa_planta, req.regimen, req.f_inicio, req.f_fin, req.causal, req.motivo, req.comentario, req.id as id_requerimiento, rasc.id as idrasc, archivo.renta_imponible');
		...
		$this->db->join('r_requerimiento_usuario_archivo archivo','rasc.id = archivo.requerimiento_asc_trabajadores_id','inner');
	...
	function get_cargo_area($id){
		$this->db->where('requerimiento_area_cargo_id',$id);
		$query = $this->db->get('r_requerimiento_asc_trabajadores');
		return $query->result();
	}
	...
	function eliminar($id){
		$this->db->delete('r_requerimiento_asc_trabajadores', array('id' => $id)); 
	}

	function editar($id,$data){
		$this->db->where('id', $id);
		$this->db->update('r_requerimiento_asc_trabajadores', $data); 
	}

	function ingresar($data){
		$this->db->insert('r_requerimiento_asc_trabajadores',$data); 
	...
	function verificarSiExiste($id_area_cargo, $idTrabajador){
		$this->db->select('*');
		$this->db->from('r_requerimiento_asc_trabajadores');
		$this->db->where('requerimiento_area_cargo_id',$id_area_cargo);
		$this->db->where('usuario_id',$idTrabajador);
		$query = $this->db->get();
		if($query->num_rows() > 0){
			return true;
		}else{
	...
	#yayo14-10-2019
	function getTrabajadoresAnexo($id){
		$this->db->select('anexos_masivos.url, anexos_masivos.nombre_archivo, usuarios.nombres, usuarios.paterno, usuarios.materno, usuarios.rut_usuario, anexos_masivos.id_trabajador');
		$this->db->from('anexos_masivos');
		$this->db->join('usuarios', 'anexos_masivos.id_trabajador = usuarios.id','inner');
		$this->db->where('anexos_masivos.id_requerimiento',$id);
		$query = $this->db->get();
		return $query->result();
	}
	function getTrabajadoresAnexoforUsuario($id){
		$this->db->select('anexos_masivos.url');
		$this->db->from('anexos_masivos');
		$this->db->where('anexos_masivos.id_trabajador',$id);
		$query = $this->db->get();
		return $query->row();
	}
	
	#yayo 16-10-2019
	function getTrabajadoresContrato($id){
		$this->db->select('contratos_masivos.url, contratos_masivos.nombre_archivo, usuarios.nombres, usuarios.paterno, usuarios.materno, usuarios.rut_usuario, contratos_masivos.id_trabajador');
		$this->db->from('contratos_masivos');
		$this->db->join('usuarios', 'contratos_masivos.id_trabajador = usuarios.id','inner');
		$this->db->where('contratos_masivos.id_requerimiento',$id);
		$query = $this->db->get();
		return $query->result();
	}

	function getTrabajadoresContratoforUsuario($id){
		$this->db->select('contratos_masivos.url');
		$this->db->from('contratos_masivos');
		$this->db->where('contratos_masivos.id_trabajador',$id);
		$query = $this->db->get();
		return $query->row();
	}
	#Prueba curl
	function guardarNew(){
		$this->db->insert('pruebajson',$data); 
		return $this->db->insert_id();
	}

	function verificarListaNegra($idTrabajador){
		$this->db->select('*');
		$this->db->from('lista_negra');
		$this->db->where('id_usuario',$idTrabajador);
