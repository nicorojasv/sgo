...
class Capacitaciones extends CI_Controller {
	...
	function index(){
		...
		$base['titulo'] = "Capacitaciones integra";
		$base['lugar'] = "Capacitaciones";
		$pagina['pag_lugar'] = "capacitaciones";
		
		$lista = array();
		foreach($this->Noticias_model->mostrar_listado_cap(2) as $n){//solo mostrara usuarios tipo 2, que son trabajadores.
			$aux = new stdClass();
	...
	function detalle($id = FALSE){
		...
		$noticia = $this->Noticias_est_model->get($id);
		$tipo_usuario = $this->Noticias_est_model->get_tu($id);
		$adjuntos = $this->Noticias_est_model->listar_adjuntos($id);

		$base = array(
			'head_titulo' => "Capacitaciones Integra",
			'titulo' => "Capacitaciones",
			'side_bar' => true,
			'js' => array('plugins/holder/holder.js'),
			'css' => array('plugins/nvd3/nv.d3.min.css'),
			'lugar' => array(array('url' => 'trabajador', 'txt' => 'Inicio'), array('url' => '', 'txt' => 'Detalle Capacitaciones')  ),
			'menu' => $this->menu
		);
		...
		$s = $this->Noticias_est_model->get_revisar($this->session->userdata('id'),$id);
