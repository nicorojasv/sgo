...
class Mensajes extends CI_Controller {
	...
	function index(){
		redirect('trabajador/mensajes/bandeja/', 'refresh');
	}
	...
	function bandeja() {
		...
		$base['lugar'] = "Mensajes";
		$base['lugar_aux'] = "<a href='".base_url()."trabajador/mensajes/crear' class='tab dialog'>Nuevo mensaje</a>";
	...
	function responder(){
		...
		$msg = $this->Mensajes_model->get($_POST['id']);
		if( $msg->id_usuario_envio == $this->session->userdata("id"))
			$no_leido = "visto_resp";
		if( $msg->id_usuario_resp == $this->session->userdata("id"))
			$no_leido = "visto_envio";
		$d = array(
			$no_leido => 0,
		);
		$this->Mensajes_model->editar($_POST['id'],$d);
		redirect(base_url()."trabajador/mensajes/detalle/".urlencode(base64_encode($_POST['id'])), 'refresh');
	}
