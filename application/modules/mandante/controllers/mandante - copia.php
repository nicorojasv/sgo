...
class Mandante extends CI_Controller{
	...
	function trazabilidad($id_planta){
		...
		if($dotacion_total == 0){
			...
			$pagina['porcentaje_referidos'] = number_format( ($referidos * 100) / $dotacion_total, 2);
		}
	...
	function base_datos_contratos($id_planta){
		...
		$requerimientos_planta = $this->Requerimientos_model->listar_datos_req_todos($id_planta);
		$listado = array();
		foreach($requerimientos_planta as $rm){
			$aux2 = new stdClass();
			$datos_req = $this->Requerimiento_asc_trabajadores_model->get_requerimiento($rm->id);
			$aux2->datos = array();
				if (!empty($datos_req)){
					foreach ($datos_req as $r) {
						$aux = new StdClass();
						...
						$aux->id = $r->id;
						$aux->id_requerimiento = $rm->id;
						$get_datos_requerimiento = $this->Requerimiento_model->r_get($rm->id);
						$planta_id_req = (isset($get_datos_requerimiento->planta_id))?$get_datos_requerimiento->planta_id:'0';
						$aux->nombre_req = (isset($get_datos_requerimiento->nombre))?$get_datos_requerimiento->nombre:'N/D';
	...
	function reportabilidad($id_planta){
		$this->load->model('Empresa_planta_model');
		$this->load->model('Requerimientos_model');
		...
		$base = array(
			...
			'side_bar' => true,
			'js' => array('js/avance_pgp.js', 'js/si_graficos_reportabilidad.js', 'js/si_canvasjs.min.js'),
			'menu' => $this->menu
		);

		$listar_contratos_planta = $this->Requerimientos_model->contratos_vigentes_planta($id_planta);
		$listar_contratos_planta_usuario = $this->Requerimientos_model->contratos_vigentes_planta_por_usuario($id_planta);
		$contratosmes1 = 0;
		$contratosmes2 = 0;
		$contratosmes3 = 0;
		$contratosmes4 = 0;
		...
		foreach($listar_contratos_planta as $rm){
			...
			if($fecha_inicio == '0000-00-00' or $fecha_termino == '0000-00-00'){
				...
				foreach($meses as $mes){
					...
					$time = strtotime('2016-'.$mes.'-01');
					$time2 = strtotime('2016-'.$mes.'-'.$ultimo_dia);
	...
	function reporte_causales($id_planta){
		$this->load->model('Empresa_planta_model');
		$this->load->model('Requerimientos_model');
		...
		$pagina['c_contratosA'] = $this->Requerimientos_model->cantidad_contratos_vigentes_causales($id_planta, $fecha_inicio, $fecha_termino, 'A')->cantidad_contratos;
		$pagina['c_contratosB'] = $this->Requerimientos_model->cantidad_contratos_vigentes_causales($id_planta, $fecha_inicio, $fecha_termino, 'B')->cantidad_contratos;
		$pagina['c_contratosC'] = $this->Requerimientos_model->cantidad_contratos_vigentes_causales($id_planta, $fecha_inicio, $fecha_termino, 'C')->cantidad_contratos;
		$pagina['c_contratosD'] = $this->Requerimientos_model->cantidad_contratos_vigentes_causales($id_planta, $fecha_inicio, $fecha_termino, 'D')->cantidad_contratos;
		$pagina['c_contratosE'] = $this->Requerimientos_model->cantidad_contratos_vigentes_causales($id_planta, $fecha_inicio, $fecha_termino, 'E')->cantidad_contratos;

		$pagina['por_trabajadorA'] = $this->Requerimientos_model->cantidad_contratos_vigentes_causales_trabajador($id_planta, $fecha_inicio, $fecha_termino, 'A');
		$pagina['por_trabajadorB'] = $this->Requerimientos_model->cantidad_contratos_vigentes_causales_trabajador($id_planta, $fecha_inicio, $fecha_termino, 'B');
		$pagina['por_trabajadorC'] = $this->Requerimientos_model->cantidad_contratos_vigentes_causales_trabajador($id_planta, $fecha_inicio, $fecha_termino, 'C');
		$pagina['por_trabajadorD'] = $this->Requerimientos_model->cantidad_contratos_vigentes_causales_trabajador($id_planta, $fecha_inicio, $fecha_termino, 'D');
		$pagina['por_trabajadorE'] = $this->Requerimientos_model->cantidad_contratos_vigentes_causales_trabajador($id_planta, $fecha_inicio, $fecha_termino, 'E');
