...
class Trabajadores extends CI_Controller{
	...
	function index(){
		...
		$base = array(
			...
			'subtitulo' => '<b>Unidad de Negocio:</b> Marina Chillán',
			'side_bar' => false,
			'js' => array('js/confirm.js','plugins/DataTables/media/js/jquery.dataTables.min.js','plugins/DataTables/FixedColumns/js/dataTables.fixedColumns.min.js','js/exportarExcelmarina_chillan.js'),
	...
	function crear(){
		...
		$this->load->model("marina_chillan/Usu_parentesco_model");
		$this->load->model("marina_chillan/Profesiones_model");

		$base = array(
			'head_titulo' => "Sistema EST - Datos Trabajador",
			'titulo' => "Empresa: Celulosa Arauco S.A.",
			'subtitulo' => 'Unidad de Negocio: Marina Chillán',
			'side_bar' => true,
			'lugar' => array(array('url' => 'usuarios/home', 'txt' => 'Inicio'), array('url'=>'marina_chillan/trabajadores','txt'=>'Listado Trabajadores' ), array('url'=>'','txt'=>'Crear Trabajador')),
		...
		$pagina['soloParamarina_chillan']= true;
		...
		$base['cuerpo'] = $this->load->view('marina_chillan/trabajadores/crear_trabajador',$pagina,TRUE);
		$this->load->view('layout2.0/layout',$base);
	}
	...
	function detalle($id){
		...
		$base = array(
			...
			'subtitulo' => 'Unidad de Negocio: Marina Chillán',
			'side_bar' => true,
			'lugar' => array(array('url' => 'usuarios/home', 'txt' => 'Inicio'), array('url'=>'marina_chillan/trabajadores','txt'=>'Listado Trabajadores' ), array('url'=>'','txt'=>'Datos Trabajador')),
			'js' => array('js/jquery.Rut.min.js','js/usuarios.js', 'js/confirm.js', 'plugins/DataTables/media/js/jquery.dataTables.min.js','plugins/DataTables/FixedColumns/js/dataTables.fixedColumns.min.js','js/ui-subview.js','plugins/select2/select2.min.js','js/si_validaciones.js'),
			'css' => array('plugins/DataTables/media/css/jquery.dataTables.min.css','plugins/DataTables/media/css/dataTables.bootstrap.min.css','plugins/DataTables/FixedColumns/css/fixedColumns.dataTables.css','plugins/select2/select2.css'),
			'menu' => $this->menu
		);
		...
		$base['cuerpo'] = $this->load->view('marina_chillan/trabajadores/detalle',$pagina,TRUE);
		$this->load->view('layout2.0/layout',$base);
	}
	...
	function eliminar_trabajador($id_usuario){
		...
		redirect('marina_chillan/trabajadores', 'refresh');
	}
	...
	function guardar_nuevo_trabajador(){
		...
		redirect('marina_chillan/trabajadores/detalle/'.$id_usuario.'#datos-personales', 'refresh');
	}
	...
	function guardar_datos_de_emergencia(){
		...
		$this->Usuarios_model->editar($id_usuario,$data);
		redirect('marina_chillan/trabajadores/detalle/'.$id_usuario.'#contacto-emergencia', 'refresh');
	}
	...
	function guardar_tecnicos(){
		...
		$this->Usuarios_model->editar($id_usuario,$data);
		redirect('marina_chillan/trabajadores/detalle/'.$id_usuario.'#datos-tecnicos', 'refresh');
	}
	...
	function guardar_extra(){
		...
		$this->Usuarios_model->editar($id_usuario,$data);
		redirect('marina_chillan/trabajadores/detalle/'.$id_usuario.'#datos-extras', 'refresh');
	}

		function contratos_y_anexos(){
		$this->load->model("marina_chillan/Usuarios_model");
		$base = array(
			'head_titulo' => "EST Marina Chillán",
			'titulo' => "Empresas: Marina Chillán",
		...
		$base['cuerpo'] = $this->load->view('marina_chillan/trabajadores/base_datos_contratos_y_anexos',$pagina,TRUE);
	 	$this->load->view('layout2.0/layout',$base);
	}
...
/*	function exportar_excel_contratos_y_anexos(){
	...
	function anotaciones($id){
		...
		$base = array(
			...
			'css' => array('plugins/DataTables/media/css/jquery.dataTables.min.css','plugins/DataTables/media/css/dataTables.bootstrap.min.css','plugins/DataTables/FixedColumns/css/fixedColumns.dataTables.css'),
			'lugar' => array(array('url' => 'usuarios/home', 'txt' => 'Inicio'), array('url'=>'marina_chillan/trabajadores','txt'=>'Listado Trabajadores' ), array('url'=>'','txt'=>'Anotaciones')),
			'menu' => $this->menu
		);
		$pagina ="";
	...
	function guardar_anotacion($id){
		...
		redirect('marina_chillan/trabajadores/anotaciones/'.$id, 'refresh');
	}
	...
	function eliminar_anotacion($id_usuario,$id_anotacion){
		...
		redirect('marina_chillan/trabajadores/anotaciones/'.$id_usuario, 'refresh');
	}
	/*END Lista Negra*/
	...
	function restriccion_de_contratacion($rutTrabajador = FALSE){
		...
		if($this->session->userdata('tipo_usuario') == 8 || $this->session->userdata('id')==105){
			...
			$base = array(
				'head_titulo' => "Sistema EST",
				'titulo' => "Empresas Integra Ltda.",
				'subtitulo' => '<b>Unidad de Negocio:</b> marina_chillan',
				'side_bar' => true,
	...
	function solicitar_liberar_lista_negra(){
		...
		$this->load->library('email');
		    ...
		    $this->email->from('informaciones@empresasintegra.cl', 'Solicitud Liberación - MDS Chillán');
		    ...
		    $this->email->message('Estimado se solicita liberación de trabajador: <b>'.$nombre.'</b> con rut: <b>'.$rut.'</b>, en MDS Chillán <a href="'.base_url().'marina_chillan/trabajadores/restriccion_de_contratacion/'.$rut.'">ir a liberar</a>');
		    $this->email->send();
		echo json_encode(1);
	}
}
