...
class Contratos extends CI_Controller{
	...
	function index(){
		redirect('marina_chillan/contratos/solicitudes_pendientes', 'refresh');
	}
	...
	function solicitudes_pendientes(){
		$base = array(
			...
			'subtitulo' => 'Unidad de Negocio: Marina Chillán Del Sol',
			'side_bar' => true,
		...
		if($trabajadores != FALSE){
			foreach($trabajadores as $rm){
				...
				if($renta_imponible == 15000){
					...
					$sueldoMensualDeAcuerdoAsuAFP = $sueldo_liquido_27mil;
					...
					$sueldoMensualDeAcuerdoAsuAFP = $sueldo_liquido_38mil;
					...
					$sueldoMensualDeAcuerdoAsuAFP = $sueldo_liquido_44mil;
					...
					$sueldoMensualDeAcuerdoAsuAFP = $sueldo_liquido_60mil;
				}elseif($renta_imponible == 70000){
					$renta_imponible = round($sueldo_liquido_70mil / 30);
					$sueldoMensualDeAcuerdoAsuAFP = $sueldo_liquido_70mil;
				}else{
					$renta_imponible = 0;
	...
	function solicitudes_completas($fecha = false){
		$base = array(
			...
			'menu' => $this->menu,
			'js' => array('js/exportarExcelmarina_chillan.js','plugins/DataTables/media/js/jquery.dataTables.min.js','plugins/DataTables/FixedColumns/js/dataTables.fixedColumns.min.js','js/si_exportar_excel.js', 'plugins/bootstrap-modal/js/bootstrap-modal.js', 'plugins/bootstrap-modal/js/bootstrap-modalmanager.js', 'js/main.js','js/evaluar_pgp.js','plugins/bootstrap-datepicker/js/bootstrap-datepicker.js'),
			'css' => array('plugins/DataTables/media/css/jquery.dataTables.min.css','plugins/DataTables/FixedColumns/css/fixedColumns.dataTables.css','plugins/bootstrap-modal/css/bootstrap-modal-bs3patch.css', 'plugins/bootstrap-modal/css/bootstrap-modal.css'),
		);
	...
	function validar_contrato_anexo_doc($id_req_usu_arch){
	    ...
	    $this->email->from('informaciones@empresasintegra.cl', 'Solicitud Contrato SGO - Marina Chillán');
	    $this->email->to($email_solicitante);
		$this->email->cc($destinatarios_cc);
	    $this->email->subject("Aprobación Contrato Trabajador: ".$nombre_trabajador." - Fecha Inicio Contrato: ".$fecha_inicio);
	...
	function modal_rechazar_contrato_anexo_doc_general($id_usu_arch){
		...
		$pagina['id_usu_arch']= $id_usu_arch;
		$pagina['datos_usu_arch']= $this->Requerimiento_Usuario_Archivo_model->get_result($id_usu_arch);
		$this->load->view('marina_chillan/contratos/modal_rechazar_contrato_anexo_doc_contractuales', $pagina);
	}
	...
	function rechazar_contrato_anexo_doc_general(){
	  ...
	  // $this->email->to($email_solicitante);
	    ...
	    $tMis->emCil->message('Estimado(a) '.$nombre_completo_solicitante.' la solicitud de contrato del trabajador: '.$nombre_trabajador.' con el siguiente Rut: '.$rut_trabajador.' ha sido rechazado.<br>Con la siguiente observacion:<br>'.$_POST['observaciones'].'.<br>Saludos');
	    $this->email->send();
	...
	function generar_contrato_anexo_doc_contractual(){
		...
		if (isset($_POST['dar_de_baja'])) { //para dar de baja un contrato
			...
			$nombre_trabajador = $nombre." ".$aP." ".$aM;
				...
				$destinatarios_cc = array('contratos@empresasintegra.cl','jsilva@empresasintegra.cl');
			    $this->email->from('informaciones@empresasintegra.cl', 'Solicitud Contrato SGO - Marina Chillán');
			    $this->email->to('supervisormdschillan@empresasintegra.cl');
				$this->email->cc($destinatarios_cc);
			    $this->email->subject("Baja de Contrato Marina Chillán Trabajador: ".$nombre_trabajador." APROBADA ");
			    $this->email->message('Estimado(a),<br> Solicitud de baja de contrato con motivo='.$motivo.' Se encuentra Aprobada.  <br>Saludos');
			    $this->email->send();
			redirect('marina_chillan/contratos/solicitudes_pendientes_baja','refresh');
			return false;
			...
			$this->load->library('email');
			    ...
			    $this->email->to('supervisormdschillan@empresasintegra.cl');
				$this->email->cc($destinatarios_cc);
			    $this->email->subject("Baja de Contrato Marina Chillán Trabajador: ".$nombre_trabajador." RECHAZADA ");
	...
	function solicitud_bajar_contrato($id_usu_arch){
		if (empty($_POST['value']) || $_POST['value']=='Seleccione Motivo de baja') {
			...
			$hoy = date('Y-m-d');
			    ...
			    $this->email->subject("Baja de Contrato Marina Chillán Trabajador: ".$nombre_trabajador." fecha: ".$hoy." PENDIENTE ");
		...
		//redirect('marina_chillan/contratos/solicitudes_completas','refresh');*/
