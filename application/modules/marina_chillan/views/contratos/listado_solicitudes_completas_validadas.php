...
<div class="panel panel-white">
  <div class="panel-body">
      <div class="row">
        ...
        <?php if (!isset($completa_baja)) { ?>
          <div class="col-md-8" align="left">
              <label for="datepickermarinaSCompletas">Mes A Consultar: </label>
              <input name="datepicker" type="text" id="datepickermarinaSCompletas" style="border: 1px solid #ccc;" class="datepicker" value="<?php if(isset($mes)) echo $mes ?>" size="10" readonly="true" title="Fecha a Gestionar Asistencia"><br>
      ...
      <label for="historico" style="cursor: pointer;" >Historico</label>
        ...
        <?php }else{ ?>
              <div class="col-md-8" align="left">
              <label for="datepickerMarinaBajadas">Mes A Consultar: </label>
              <input name="datepicker" type="text" id="datepickerMarinaBajadas" style="border: 1px solid #ccc;" class="datepicker" value="<?php if(isset($mes)) echo $mes ?>" size="10" readonly="true" title="Fecha a Gestionar Asistencia"><br>
        ...
        <?php }?>
          <div class="col-md-4" align="center">
            <form action="<?php echo base_url() ?>marina_chillan/contratos/exportar_excel_contratos_y_anexos" method="post" target="_blank" id="FormularioExportacion">
              <input title="EXPORTAR DATOS A ARCHIVO EXCEL" type="button" class="botonExcelmarina btn btn-success btn-block" value="Exportar a Excel"><br>
          ...
          <table id="example1">
            ...
            <tbody>
              ...
              <tr id="<?php echo $i ?>">
                ...
                ?>
                <td>
                  <a data-toggle="modal" href="<?php echo base_url() ?>marina_chillan/contratos/modal_visualizar_contrato_anexo_doc_general/<?php echo $rm->id_req_usu_arch ?>" data-target="#ModalEditar" title="Visualizar mas detalles"><i class="fa fa-search" aria-hidden="true"></i></a>
                  <?php if($this->session->userdata('tipo_usuario') == 16 || $this->session->userdata('tipo_usuario') == 8   || $this->session->userdata('id') == 140){?>

                  <i class="fa fa-minus" style="color: #ccc" aria-hidden="true"></i>
                 <!-- <a href="<?php echo base_url() ?>marina_chillan/contratos/solicitud_bajar_contrato/<?php echo $rm->id_req_usu_arch ?>" title="Solicitar baja contrato" onClick="if(confirm('¿Esta seguro de dar de baja este contrato ?')) return true; else return false;"><i class="fa fa-sign-out" style="color: red" aria-hidden="true"></i></a>-->
        ...
        var idQuitar = $(this).attr('data-quitar');
          alertify.prompt('Baja de Contrato',"Contrato de "+nombre+".<br><br>Especifique motivo de solicitud de baja: <select id ='motivoBaja' onchange='remove(this.value);'><option>Seleccione Motivo de baja</option><option>Inasistencia con aviso</option><option>Inasistencia sin aviso</option><option>Cambio de Fecha</option><option>Cambio de Horario</option><option>Cambio de Renta</option><option>Cambio de Cargo</option><option>Modificación Motivo</option><option>Modificación Causal</option><option>Presentarse sin Uniforme</option><option>Cambio de Ceco y/o Area</option><option>Anulación requerimiento</option><option>Duplicidad</option><option>Otro</option></select>", "",
            function(evt, value ){// si presiona ok hacer esto
              $.ajax({
                  type: "POST",
                  url: base_url+"marina_chillan/contratos/solicitud_bajar_contrato/"+service,
