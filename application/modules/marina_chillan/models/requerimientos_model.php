...
class Requerimientos_model extends CI_Model{
	...
	function ingresar($data){
		$this->marina_chillan->insert('r_requerimiento',$data);
		return $this->marina_chillan->insert_id();
	}

	function r_listar_order_estado(){
		$this->marina_chillan->order_by("estado", "desc"); 
		$query = $this->marina_chillan->get('r_requerimiento');
		return $query->result();
	}

	function actualizar_desactivo_requerimiento($datos){
		$this->marina_chillan->update('r_requerimiento', $datos);
	}
	
	function actualizar($id_req, $data){
		$this->marina_chillan->where('id', $id_req);
		$this->marina_chillan->update('r_requerimiento', $data);
	}

	function eliminar($id){
		$this->marina_chillan->delete('r_requerimiento', array('id' => $id)); 
	}
	...
	function r_listar_order_estado_fecha($fecha_inicio, $fecha_termino){
		...
		$this->marina_chillan->from("r_requerimiento"); 	
		$this->marina_chillan->order_by("id", "desc"); 
		//$this->marina_chillan->limit(10); 
		$this->marina_chillan->where('f_inicio BETWEEN "'.$fecha_inicio. '" and "'. $fecha_termino.'"');
	...
	function get_req_planta($id){
		$this->marina_chillan->select("*");
		$this->marina_chillan->from('r_requerimiento');
		$this->marina_chillan->join('empresa_planta', 'empresa_planta.id = r_requerimiento.planta_id');
		$this->marina_chillan->where("r_requerimiento.id",$id);
		$query = $this->marina_chillan->get();
		return $query->row();
	}
	...
	function getRequerimientoPuesto($id){
		$this->marina_chillan->select('rr.id, 
						   ...
						   empresa.rut as rutEmpresa, 
						   empresa.nombre_gerente as nameGerente,
						   empresa.rut_gerente as rutGerente,
						  ...
						  ');
		$this->marina_chillan->from('r_requerimiento rr');
		$this->marina_chillan->join('empresa','empresa.id = rr.empresa_id','inner');
		$this->marina_chillan->join('empresa_planta','empresa_planta.id = rr.planta_id','inner');
		$this->marina_chillan->join('ciudades','ciudades.id = empresa_planta.id_ciudades','inner');
		$this->marina_chillan->where('rr.id',$id);
		$query = $this->marina_chillan->get();
		return $query->row();
	}
