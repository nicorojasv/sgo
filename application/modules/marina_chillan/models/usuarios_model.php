...
class Usuarios_model extends CI_Model {
	...
	function guardarAnotacion($id, $valor){
		$this->marina_chillan->where('id', $id);
		$this->marina_chillan->update('usuarios', $valor); 
		return $afftectedRows = $this->marina_chillan->affected_rows();
	}

	function getCargos(){
		$this->marina_chillan->select();
		$this->marina_chillan->from('r_cargos');
		$query = $this->marina_chillan->get();
		return $query->result();
	}
	...
	function verficarListaNegra($id){
		...
		$this->marina_chillan->from('lista_negra');
		$this->marina_chillan->where("id_usuario",$id);
		$this->marina_chillan->where("estado",NULL);//-> Edit 21-01-2019
		$query = $this->marina_chillan->get();
		if($query->num_rows() > 0){
			return true;
		}else{
	...
	function getAllListaNegra(){
		$this->marina_chillan->select('*');
		$this->marina_chillan->from('lista_negra');
		$this->marina_chillan->where('estado',NULL);
		$query = $this->marina_chillan->get();
		return $query->result();
	}
	function updateListaNegra($id,$data){
		$this->marina_chillan->where('id', $id);
		$this->marina_chillan->update('lista_negra', $data); 
		return $afftectedRows = $this->marina_chillan->affected_rows();
	}
