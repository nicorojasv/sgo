...
function desactivar_input_cc(){
  document.getElementById("ccosto").disabled = true;
  document.getElementById("valor_examen").disabled = true;
  document.getElementById("oc").disabled = true;
  document.getElementById("ciudadch").disabled = true;
}

function activar_input_cc(){
  document.getElementById("ccosto").disabled = false;
  document.getElementById("valor_examen").disabled = false;
  document.getElementById("oc").disabled = false;
  document.getElementById("ciudadch").disabled = false;
}
</script>
...
<div class="col-md-10 col-md-offset-2">
	...
	<h3>A&ntilde;adir Evaluaci&oacute;n Masso</h3>
	...
	<div class='col-md-6'>
		...
		<div class="form-group">
			<div class="col-sm-10">
				<label class="radio-inline">
				   <input type="radio" id="pago" name="pago" value='0' <?php if($eval){ echo ($eval->pago == 0) ? "checked='checked'" : ''; } ?> onclick="activar_input_cc()" checked/> Propia
				</label>
				<label class="radio-inline">
				   <input type="radio" id="pago" name="pago" value='1' <?php if($eval){ echo ($eval->pago == 1) ? "checked='checked'" : ''; } ?> onclick="desactivar_input_cc()"/> Terceros
				</label>
			</div>
			<!-- .fields -->
		</div>
		<div class="form-group">
			...
			<div class="col-sm-4">
				...
				?>
					<input type="text" id="valor_examen" name="valor_examen" value="<?php if($eval){ echo ($eval->valor_examen) ? $eval->valor_examen : '0'; } ?>" required readonly="readonly">
				<?php
					}else{
				?>
					<input type="text" id="valor_examen" name="valor_examen" value="<?php if($eval){ echo ($eval->valor_examen) ? $eval->valor_examen : '0'; } ?>" required>
				<?php
					}
				}else{
				?>
		...
		<div class="form-group">
			...
			<div class="col-sm-4">
				<input type="text" id="oc" name="oc" value="<?php if($eval){ echo ($eval->oc) ? $eval->oc : ''; } ?>" >
			</div>
		</div>
		<div class="form-group">
		...
		<div class="form-group">
			...
			<div class="col-sm-4">
				<input type="text" id="ciudadch" name="ciudadch" value="<?php if($eval){ echo ($eval->ciudadch) ? $eval->ciudadch : ''; } ?>" >
			</div>
		</div>
	</div>
	</form>
