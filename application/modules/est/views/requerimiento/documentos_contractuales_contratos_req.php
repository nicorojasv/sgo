	...
	<div class="panel-body">
		...
		<div class="col-md-12" align="right">
			<?php 
				if (empty($contratos)) {
			?>
          	<a data-toggle="modal" href="<?php echo base_url() ?>est/requerimiento/modal_agregar_contrato_anexo/<?php echo $id_usuario ?>/1/<?php echo $id_asc_area_req ?>/<?php echo $id_area_cargo_req ?>" data-target="#ModalEditar"><input type="button" class='btn btn-blue' value="Agregar Contrato"></a>
			...
			<?php 
					    ...
					    //if($limitAnexo==false){
			?>          
          				<a data-toggle="modal" href="<?php echo base_url() ?>est/requerimiento/modal_agregar_anexo/<?php echo $id_usuario ?>/2/<?php echo $id_asc_area_req ?>/<?php echo $id_area_cargo_req ?>" id="AgregarAnexo" data-target="#ModalAnexo"><input type="button" class='btn btn-green' value="Agregar Anexo"></a>
          	...
          	<?php   
          			...
          			if ($idArchivo != null) {
          	?>
						<a  href="<?php echo base_url() ?>est/requerimiento/descargar_carta_termino/<?php echo $id_usuario ?>/<?php echo $idArchivo ?>/<?php echo $tipoArchivo ?>"><i title="Descargar Carta Termino" class="btn fa fa-envelope" aria-hidden="true"></i></a>
          	<?php
          			}
          	?>
			<br><br>
				...
				<tbody>
					...
					?>
						<tr>
    	                    ...
    	                    <td align="center">
    	                    	<a data-toggle="modal" href="<?php echo base_url() ?>est/requerimiento/modal_administrar_contrato_anexo_doc_general/<?php echo $row->id_req_usu_arch ?>/<?php echo $id_area_cargo_req ?>" data-target="#ModalEditar2"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
    	                    <?php 
    	                    	if ($row->url) {
    	                    ?>
    	                    ...
    	                    ?>
    	                    |	<a title="Subir Contrato Firmado"  data-toggle="modal" href="<?php echo base_url() ?>est/requerimiento/modal_administrar_archivo_usu/<?php echo $row->id_req_usu_arch ?>/<?php echo $row->idAreaCargo ?>" data-target="#ModalEditar2"><i style="color: black" class="fa fa-cloud-upload" aria-hidden="true"></i></a>
    	                    <?php
    	                    	}
					...
					<?php foreach ($anexos as $row) { ?>
						<tr id="<?php echo $row->id_req_usu_arch.'tr'; ?>">
							...
							<td align="center"  id="<?php echo $row->id_req_usu_arch.'td'; ?>"<?php if ($row->estado == 6) { ?> style=" border: 1px solid #ff0000; " title="Rechazado " <?php }?>>
    	                    	...
    	                    	?>
    	                    			<a data-toggle="modal" title="Modificar Fecha Termino" href="<?php echo base_url() ?>est/requerimiento/modal_administrar_anexo/<?php echo $row->id_req_usu_arch ?>/<?php echo $id_area_cargo_req ?>" data-target="#ModalAnexo"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a> |
    	                    		<?php 
    	                    		   #QuitarEnCasoDeErrorEl# 
    	                    		   if(isset($contrato)){}else{$contrato=1;}
    	                    		if ($contrato ==1) { ?>
    	                    		...
    	                    		<?php }?>
    	                    			<!--<a href="<?php echo base_url() ?>est/requerimiento/eliminar_anexo/<?php echo $row->id_req_usu_arch ?>" title="Eliminar Anexo"><i style="color:red;" class="fa fa-times" aria-hidden="true"></i></a>-->
    	                    	...
    	                    	?>
    	                    			<a href="<?php echo base_url() ?>est/requerimiento/descargar_anexo/<?php echo $row->id_req_usu_arch ?>" title="Descargar Anexo"><i style="color: green" class="fa fa-download" aria-hidden="true"></i></a>
    	                    		<?php 
			    	                    	if ($row->url) {
			    	                    ?>
			    	                    ...
			    	                    ?>
			    	                    |	<a title="Subir Anexo Firmado"  data-toggle="modal" href="<?php echo base_url() ?>est/requerimiento/modal_cargar_anexo/<?php echo $row->id_req_usu_arch ?>" data-target="#ModalEditar2"><i style="color: black" class="fa fa-cloud-upload" aria-hidden="true"></i></a>
			    	                <?php
			    	                    	}
			    	                ?>
    	                    	<?php
    	                    	...
    	                    	if ($row->fecha_termino2> date('Y-m-d') && $row->estado == 2) {
    	                    	?>
	...
	function confirmar(e){//Funcion de confirmar eliminacion de Anexo
		...
		var fechaTermino = $(e).attr('data-fecha');
		  	...
		  	function(){ //Si confirmo 
		  		        $.ajax({
				            type: "POST",
				            url: base_url+"est/requerimiento/eliminar_anexo/"+id,
				            data: id,
				            dataType: "json",
		...
		var fechaTermino = $(e).attr('data-fecha');
		  	...
		  	function(){ //Si confirmo 
		  		$('#content').html("<div class='loading'><img src='"+base_url+"extras/img/loader2.gif' alt='loading' /><br/>Un momento, por favor...</div>");
		  		        $.ajax({
				            type: "POST",
				            url: base_url+"est/requerimiento/enviar_revision/"+id,
				            data: id,
				            dataType: "json",
				            success: function(data) {  
					            if (data ==1) {
...
$(document).on('click', '.enviarFechaFiniquito', function() {
	    $.ajax({
            type: 'POST',
            url: base_url+'est/requerimiento/guardar_fecha_finiquito',
            data: {
