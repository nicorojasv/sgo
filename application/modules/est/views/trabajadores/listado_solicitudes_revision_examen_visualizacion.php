<div class="panel panel-white">
	...
	<div class="col-md-8" align="left">
              <label for="datepickerPsicologos">Mes A Consultar: </label>
              <input name="datepicker" type="text" id="datepickerPsicologos" style="border: 1px solid #ccc;" class="datepicker" value="<?php if(isset($mes)) echo $mes ?>" size="10" readonly="true" title="Fecha a Gestionar Asistencia"><br>
								...
								<?php if($row->examen_masso != 0){ ?>
									...
									<?php }else{
										if($row->id_sre_agenda_masso != NULL)
											echo "<span class='badge' style='background-color:#D7DF01'>EP</span><a href='".base_url()."est/trabajadores/modal_agendar_examen/".$row->id_solicitud."/1/1' data-toggle='modal' data-target='#ModalEditar'><font color='blue'>Agendado</a></font>";
										else
								...
								<?php if($row->examen_preo != 0){ ?>
									...
									<?php }else{
										if($row->id_sre_agenda_preo != NULL)
											echo "<span class='badge' style='background-color:#D7DF01'>EP</span><a href='".base_url()."est/trabajadores/modal_agendar_examen/".$row->id_solicitud."/2/1' data-toggle='modal' data-target='#ModalEditar'><font color='blue'>Agendado</font></a>";
										else
											echo "<span class='badge' style='background-color:#D7DF01'>EP</span><br><font color='red'>No Agendado</font>";
									} ?>
								<?php } ?>
							...
							<td style="text-align:center">
								<?php if($row->examen_psicologico != 0){ ?>
									<?php if($row->eval_psic_id != NULL){ ?>
										...
										<a href="<?php echo base_url() ?>est/examen_psicologico/detalle/<?php echo $row->eval_psic_id ?>" style='color:<?php echo $row->color_psic ?>' target="_blank"><?php echo $row->examen_psic; ?></a><br>
										<?php }else{ ?>
									...
									<?php }else{
										if($row->id_sre_agenda_psic != NULL)
											echo "<span class='badge' style='background-color:#D7DF01'>EP</span><a href='".base_url()."est/trabajadores/modal_agendar_examen/".$row->id_solicitud."/3/1' data-toggle='modal' data-target='#ModalEditar'><font color='blue'>Agendado</font></a>";
										else
											echo "<span class='badge' style='background-color:#D7DF01'>EP</span><br><font color='red'>No Agendado</font>";
									} ?>
								<?php } ?>
