...
class Contratos extends CI_Controller{
	...
	function index(){
		redirect('est/contratos/solicitudes_pendientes', 'refresh');
	}
	...
	function solicitudes_pendientes($get_id_planta = FALSE){
		...
		if($trabajadores != FALSE){
			foreach($trabajadores as $rm){
				...
				$aux->correo = isset($usr->email)?$usr->email:'';
	...
	function aprobar_contrato_anexo_doc_general($id_req_usu_arch){
		...
		$usr = $this->Usuarios_model->get($id_usuario);
		$rut_trabajador = isset($usr->rut_usuario)?$usr->rut_usuario:'';
		$rut_trabajador2 = isset($usr->rut_usuario)?$usr->rut_usuario:'';
		$nombres = isset($usr->nombres)?$usr->nombres:'';
		$paterno = isset($usr->paterno)?$usr->paterno:'';
		$materno = isset($usr->materno)?$usr->materno:'';
		$nombre_trabajador = $nombres.' '.$paterno.' '.$materno;
		...
		if($get_usu_archivo->jornada == "1"){
			if ($id_centro_costo==2 || $id_centro_costo == 4 || $id_centro_costo==5) {
				$template_formato = base_url()."extras/contratos/formatos_contratos_est/contrato_sin_pacto_masivo.docx";
				$parrafoQuinto=', con excepción del primer mes de contrato oportunidad en que se le pagara el día 5 del mes siguiente.';
			}else{
			...
			if ($id_centro_costo==2 || $id_centro_costo == 4 || $id_centro_costo==5) {
				$template_formato = base_url()."extras/contratos/formatos_contratos_est/contrato_con_pacto_masivo.docx";
				$parrafoQuinto=', con excepción del primer mes de contrato oportunidad en que se le pagara el día 5 del mes siguiente.';
			}else{
		...
		$get_estado_civil = $this->Estadocivil_model->get($usr->id_estadocivil);
		...
		//yayo 03-01-2019 nuevos para adjuntos masivos
     ...
     /*  $url = 'http://54.232.203.239/wsIntegra/insertDocumento';
		...
		$ch = curl_init($url);
		//DATOS A INGRESAR
		$nacionalidadJson = titleCase($usr->nacionalidad);
		$estadoCivilJson =  titleCase($get_estado_civil->desc_estadocivil);
		$domicilioJson = titleCase($usr->direccion);
		$comunaJson = titleCase($get_ciudad->desc_ciudades);
		$ccostoJson = titleCase($nombre_centro_costo);
		$motivoJson = titleCase($get_usu_archivo->motivo);
		$cargoJson = titleCase($get_cargo->nombre);
		$plantaJson =  titleCase($nombre_planta);
		$direccionPlantaJson = titleCase($direccion_planta);
		$ciudadPlantaJson = titleCase($ciudad_planta);
		$regionPlantaJson = titleCase($region_planta);
		$sueldoJson = str_replace(',','.',number_format($get_usu_archivo->renta_imponible));
		$data = array(
                'fecha_ingreso_trabajador_palabras' => $fecha_inicio_texto_largo,
                'nombre_trabajador' => $nombre_trabajador,
                'rut_trabajador' => $rut_trabajador2,
                'nacionalidad' => $nacionalidadJson,
                'fecha_nacimiento' => $fecha_nacimiento_texto_largo,
                'estado_civil' => $estadoCivilJson,
                'domicilio_trabajador' => $domicilioJson,
                'comuna_trabajador' => $comunaJson,
                'nombre_centro_costo' => $ccostoJson,
                'rut_centro_costo' => $rut_centro_costo,
                'descripcion_causal' => $descripcion_causal,
                'motivo_req' => $motivoJson,
                'cargo_postulante' => $cargoJson,
                'centro_costo' => $nombre_centro_costo,
                'nombre_planta' => $plantaJson,
                'direccion_planta' => $direccionPlantaJson,
                'comuna_planta' => $ciudadPlantaJson,
                'region_planta' => $regionPlantaJson,
                'descripcion_jornada' => $descripcion_jornada,
                'sueldo_base_numeros' => $sueldoJson,
                'sueldo_base_palabras' => titleCase($sueldo_base_palabras),
                'gratificacion' => $tipo_gratificacion,
                'detalle_bonos' => $detalle_bonos,
                'adicional_cumplimiento_horario_undecimo' => $adicional_cumplimiento_horario_undecimo,
                'prevision_trabajador'=> titleCase($prevision),
                'salud_trabajador' => titleCase($salud),
                'fecha_ingreso_trabajador' => $fecha_inicio_texto_largo,
                'fecha_vigencia_contrato' => $fecha_termino_texto_largo,
                'parrafo_decimo_tercero' => $parrafo_decimo_tercero,
                'nombre_planta_may' => strtoupper($nombre_planta),
                'nombre_centro_costo_may'=> strtoupper($nombre_centro_costo),
                'parrafoQuinto' => $parrafoQuinto,
                'id_negocio'=>1,// id del holding en este caso 1
                'id_planta'=>1, //las sucursales
                'id_cliente'=>2,// id de la empresa dentro del holding
                'id_documento'=>$id_req_usu_arch,// id del contrato en  SGO
                'tipo_documento'=>1, // 1 si es Contrato  2 si es Anexo
                'pacto' =>true, // si es TRUE tomas la plantilla con PACTO 
                );
	...
	//	return false;
		...
		$pagina['horarios_planta'] = $this->Descripcion_horarios_model->listar_planta($id_planta);
		$pagina['datos_usu_arch']= $this->Requerimiento_Usuario_Archivo_model->get_result($id_usu_arch);
		$this->load->view('est/contratos/modal_administrar_contrato_anexo_doc_contractuales', $pagina);
	}

	function generar_contrato_anexo_doc_contractual(){
		$this->load->library('zip');
		$this->load->helper('download');
		$id = $_POST['id_req_usu_arch'];
		$causal = $_POST['causal'];
		$motivo = $_POST['motivo'];
		...
		if (isset($_POST['dar_de_baja'])) { //para dar de baja un contrato
			$get_usu_archivo = $this->Requerimiento_Usuario_Archivo_model->get($id);
			...
			$this->Requerimiento_Usuario_Archivo_model->actualizar_solicitud_contrato($id, $data);
			...
			/*	$this->load->library('email');
			    ...
			    $this->email->send();*/
			redirect('est/contratos/solicitudes_pendientes_baja','refresh');
			return false;
			...
			if($id_planta == 2 or $id_planta == 13 )
		...
		if($_POST['solicitudes']){
			foreach($_POST['solicitudes'] as $c=>$valores){
				...
				//yayo 03-01-2019 nuevos para adjuntos masivos
					$templateWord->setValue('nombre_planta_may',strtoupper($nombre_planta));
					$templateWord->setValue('nombre_centro_costo_may',strtoupper($nombre_centro_costo));
					$templateWord->setValue('parrafoQuinto',strtoupper($parrafoQuinto));
				...
				//primero verifico si ya existe el registro asi solo actualizo
				$existeContratoMasivo = $this->Requerimiento_Usuario_Archivo_model->verContratoAprobado($id_requerimiento, $usr->id);
				if ($existeContratoMasivo == true) {//en caso de que ya existe el registro se actualiza en la tabla contratos_masivos
					$this->Requerimiento_Usuario_Archivo_model->updateContratoAprobado($id_requerimiento, $usr->id, $informacion);
				}else{
					$this->Requerimiento_Usuario_Archivo_model->insertarContratoAprobado($informacion);
				}
				$this->load->library('email');
	...
	#Solicitud baja de contrato 22-09-2020 n.r.v
	...
	*/
	...
	//inicio de boton generar anexo
		...
		$motivoNuevoAnexo =$this->Requerimiento_Usuario_Archivo_model->getMotivoAnexo($id);
				$salto_linea = "<w:br/>";
				if ($motivoNuevoAnexo) {
					$nuevoParrafo='Primero: De común acuerdo, las partes han convenido en ampliar el Contrato de Trabajo suscrito con fecha '.$fechaInicioContrato.' quedando con fecha de término el '.$fechaTerminoAnexo.$salto_linea.$salto_linea.'Segundo: Se establece que a partir de esta fecha el contrato del trabajador que esta por causal A, ahora será basado en el siguiente motivo: '.$motivoNuevoAnexo->motivo.'';
				}else{
	...
	function aprobar_anexo($id){
		...
		$email_solicitante = isset($get_solicitante->email)?$get_solicitante->email:'';
				...
				$config['smtp_port']    = '2552';
				$this->email->initialize($config);
			    $this->email->from('informaciones@empresasintegra.cl', 'Solicitud Anexo de Contrato SGO - ARAUCO');
			    $this->email->to($email_solicitante);
				$this->email->cc('contratos@empresasintegra.cl');
			    ...
			    $this->email->send();
	/*	 $url = 'http://54.232.203.239/wsIntegra/insertAnexo';
		$ch = curl_init($url);
		//DATOS A INGRESAR 
		$salto_linea = "<w:br/>";
		$nuevoParrafo='De común acuerdo, las partes han convenido en ampliar el Contrato de Trabajo suscrito con fecha '.$fechaInicioContrato.' quedando con fecha de término el '.$fechaTerminoAnexo.'.';
		$data = array(
                ...
                'fechaTerminoContratoAnterior' => $fechaTerminoContratoAnterior,
                'nombreTrabajador' => $datos->nombres,
                'rutTrabajador' => $datos->rut_usuario,
                'nacionalidadTrabajador' => $datos->nacionalidad,
                'nacimientoTrabajador' => $fechaNacimientoTrabajador,
                'civil' => $datos->estado_civil,
                'domicilioTrabajador' => titleCase($datos->direccion),
                'ciudad' => titleCase($datos->ciudad),
                'fechaInicioContrato' => $fechaInicioContrato,
                'fechaTerminoAnexo' => $fechaTerminoAnexo,
                'nuevoParrafo'=> $nuevoParrafo,
                'id_negocio'=>1,// id del holding en este caso 1
                'id_planta'=>41,
                'id_cliente'=>5,// id de la empresa dentro del holding
                'id_documento'=>$id,// id del contrato en  SGO
                //'tipo_documento'=>2, // Anexo
                );
		$token = 'v6WU6haL&Qzq=*';
	...
	//	var_dump($data);
        ...
        curl_close($ch);
        //print_r($response);*/
		redirect('est/contratos/solicitudes_pendientes_anexo/', 'refresh');
	}
