      ...
      $(".inptCheck").click(function(){
        ...
        if (this.checked == true) {
             if (this.value == 1) {//si hay que verificar el examen preocupacion
                $.ajax({
                      type:"POST",
                      url: base_url+"wood/requerimientos/verificarExamenPreocupacional/"+idPersona,
                      data:idPersona,
                      dataType: "json",
             ...
             if (this.value == 2) {//si hay que verificar charla masso
                 $.ajax({
                      type:"POST",
                      url: base_url+"wood/requerimientos/verificarCharlaMasso/"+idPersona,
                      data:idPersona,
                      dataType: "json",
                      success:function(data){
                        if (data.length ==0) {
             ...
             /*if (this.value == 3) {//si hay que verificar examen psicologico
                 $.ajax({
                      type:"POST",
                      url: base_url+"wood/requerimientos/verificarExamenPsicologico/"+idPersona,
                      data:idPersona,
                      dataType: "json",
                      success:function(data){
                       if (data.length ==0) {
...
alertify.confirm('Hola <?php//echo $this->session->userdata('nombres');?>',pre, function(){
      ...
      var idPersona=1;
                     $.ajax({
                      type:"POST",
                      url: base_url+"wood/requerimientos/no_mostrar_notificacion",
                      data:idPersona,
                      dataType: "json",
                      success:function(data){
                        if (data== true) {
...
<div id="modal">
  <form action="<?php echo base_url() ?>wood/requerimientos/guardar_nuevo_contrato_anexo_doc_contractual/<?php echo $usuario?>/<?php echo $tipo?>/<?php echo $asc_area?>" role="form" id="form2" method='post' name="f2" enctype="multipart/form-data">
