				...
				<tr>
					...
					<td class="center">
						<?php if( $a->datos ){ ?>
							<?php foreach($a->datos as $ar){ ?>
								...
								<?php }elseif ($a->id=='2') { ?>
									<a class="confirm" href="<?php echo base_url().'est/requerimiento/eliminar_anexo_subido/'.$ar->id ?>"><i class="fa fa-times"></i></a>
								<?php }?>
					...
					<td>
						<form method="post" enctype="multipart/form-data" action="<?php echo base_url() ?>est/requerimiento/guardar_doc_contractual/<?php echo $a->usuario_id ?>/<?php echo $a->id ?>/<?php echo $asc_area ?>" >
							...
							?>
							<?php foreach($a->datos as $ar){ ?>
    	                    	<a data-toggle="modal" href="<?php echo base_url() ?>est/requerimiento/modal_administrar_archivo_usu/<?php echo $ar->id ?>/<?php echo $a->id_requerimiento ?>" data-target="#ModalEditar"><input type="button" class="btn btn-xs btn-primary" value="Cargar	<?php echo $a->archivo ?> "></a><br>
							<?php } ?>
							<?php
								}elseif($a->id =="2"){
							?>
							<?php foreach($a->datos as $ar){ ?>
    	                    	<a data-toggle="modal" href="<?php echo base_url() ?>est/requerimiento/modal_cargar_anexo/<?php echo $ar->id ?>/<?php echo $a->id_requerimiento ?>" data-target="#ModalAgregarAnexo"><input type="button" class="btn btn-xs btn-primary" value="Cargar	<?php echo $a->archivo ?> "></a><br>
							<?php } ?>
							<?php
								}elseif($a->id == "13"){
							?>
