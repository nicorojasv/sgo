...
class Asistencia_model extends CI_Model {
	...
	function get_trabajadores_activo_asistencia(){
		$this->wood->select('*');
		$this->wood->from('usuarios');
		$this->wood->where('estadoAsistencia', 1);
		$this->wood->order_by('id','asc');
		$query = $this->wood->get();
		return $query->result();
	}
	...
	function actualizarAsistenciaTrabajador($idTrabajador, $data, $fechaSeleccionada){
		$this->wood->where('id_usuario', $idTrabajador);
		$this->wood->where('fecha', $fechaSeleccionada);
		$this->wood->update('asistencia', $data); 
	}
	#30-07-2018
	function verificarSiYaGuardadoBonoDelMes($fecha){
		$this->wood->select('*');
		$this->wood->from('bonos_anticipos');
		$this->wood->where("fecha",$fecha);
		$query = $this->wood->get();
		if ($query->num_rows() > 0){
			return true;
	...
	function guardarBonoTrabajador($data){
		$this->wood->insert('bonos_anticipos',$data);
	}
	...
	function actualizarBonoTrabajador($idTrabajador, $data, $fechaSeleccionada){
		$this->wood->where('id_usuario', $idTrabajador);
		$this->wood->where('fecha', $fechaSeleccionada);
		$this->wood->update('bonos_anticipos', $data); 
	}
}
?>