...
class Trabajadores extends CI_Controller{
	public function __construct(){
		...
		elseif( $this->session->userdata('tipo_usuario') == 18)
			$this->menu = $this->load->view('layout2.0/menus/menu_wood','',TRUE);
	...
	function index(){
		...
		$base = array(
			...
			'subtitulo' => '<b>Unidad de Negocio:</b> wood',
			'side_bar' => true,
		...
		$base['cuerpo'] = $this->load->view('wood/trabajadores/listado_general',$pagina,TRUE);
	...
	function crear(){
		...
		$this->load->model("wood/Usu_parentesco_model");
		$this->load->model("wood/Profesiones_model");

		$base = array(
			'head_titulo' => "Sistema EST - Datos Trabajador",
			'titulo' => "Empresa:  Wood S.A.",
			'side_bar' => true,
		...
		$pagina['lista_profesiones'] = $this->Profesiones_model->listar();
		$base['cuerpo'] = $this->load->view('wood/trabajadores/crear_trabajador',$pagina,TRUE);
		$this->load->view('layout2.0/layout',$base);
	}
	...
	function detalle($id){
		...
		$base = array(
			...
			'titulo' => "Empresa:Wood S.A.",
			'side_bar' => true,
			'lugar' => array(array('url' => 'usuarios/home', 'txt' => 'Inicio'), array('url'=>'wood/trabajadores','txt'=>'Listado Trabajadores' ), array('url'=>'','txt'=>'Datos Trabajador')),
		...
		$base['cuerpo'] = $this->load->view('wood/trabajadores/detalle',$pagina,TRUE);
		$this->load->view('layout2.0/layout',$base);
	}
	...
	function eliminar_trabajador($id_usuario){
		...
		redirect('wood/trabajadores', 'refresh');
	}
	...
	function guardar_nuevo_trabajador(){
		...
		redirect('wood/trabajadores/detalle/'.$id_usuario.'#datos-personales', 'refresh');
	}
...
,			'fono' => $_POST['fono1'],
	...
	function guardar_datos_de_emergencia(){
		...
		$this->Usuarios_model->editar($id_usuario,$data);
		redirect('wood/trabajadores/detalle/'.$id_usuario.'#contacto-emergencia', 'refresh');
	}
	...
	function guardar_tecnicos(){
		...
		$this->Usuarios_model->editar($id_usuario,$data);
		redirect('wood/trabajadores/detalle/'.$id_usuario.'#datos-tecnicos', 'refresh');
	}
	...
	function guardar_extra(){
		...
		$this->Usuarios_model->editar($id_usuario,$data);
		redirect('wood/trabajadores/detalle/'.$id_usuario.'#datos-extras', 'refresh');
	}

		function contratos_y_anexos(){
		$this->load->model("wood/Usuarios_model");
		$base = array(
			'head_titulo' => "EST wood",
			'titulo' => "Empresas: wood",
		...
		$base['cuerpo'] = $this->load->view('wood/trabajadores/base_datos_contratos_y_anexos',$pagina,TRUE);
	 	$this->load->view('layout2.0/layout',$base);
	}
...
/*	function exportar_excel_contratos_y_anexos(){
	...
	function anotaciones($id){
		...
		$base = array(
			...
			'lugar' => array(array('url' => 'usuarios/home', 'txt' => 'Inicio'), array('url'=>'wood/trabajadores','txt'=>'Listado Trabajadores' ), array('url'=>'','txt'=>'Anotaciones')),
			'menu' => $this->menu
		);
		$pagina ="";
	...
	function guardar_anotacion($id){
		...
		redirect('wood/trabajadores/anotaciones/'.$id, 'refresh');
	}
	...
	function eliminar_anotacion($id_usuario,$id_anotacion){
		...
		redirect('wood/trabajadores/anotaciones/'.$id_usuario, 'refresh');
	}
	/*END Lista Negra*/
	function crear_examen($id,$id_evaluacion=FALSE){
		...
		$this->load->model("wood/Evaluaciones_model");
		$this->load->model("wood/Usuarios_model");
		$this->load->model("wood/Evaluacionestipo_model");
		$this->load->model("wood/Evaluacionesevaluacion_model");
		$this->load->model("wood/Evaluacionesbaterias_model");
		$this->load->model("wood/Evaluacionescargos_model");
		$usr = $this->Usuarios_model->get($id);
		$base = array(
			'head_titulo' => "Sistema EST",
			'titulo' => "Empresa: WOOD S.A.",
			'side_bar' => true,
			'lugar' => array(array('url' => 'usuarios/home', 'txt' => 'Inicio'), array('url'=>'wood/trabajadores/detalle/'.$id.'','txt'=>'Datos Trabajador '.$usr->nombres.'' ), array('url'=>'','txt'=>'Agregar Examen Preocupacional' )),
		...
		$base['cuerpo'] = $this->load->view('wood/trabajadores/crear_examen',$pagina,TRUE);
		$this->load->view('layout2.0/layout',$base);
	}
	...
	function guardar_creacion_eval($id_usr, $id_evalua=FALSE){
		$this->load->model("wood/Usuarios_model");
		$this->load->model("wood/Evaluaciones_model");
		$this->load->model("wood/Evaluacionesarchivo_model");
		$this->load->model("wood/Evaluacionesbaterias_model");
		$this->load->model("wood/Evaluacionescargos_model");
		$this->load->helper("archivo");

		if(empty($id_usr)){
			redirect('wood/trabajadores/detalle/'.$id_usr.'#datos-examenes', 'refresh');
		}
	...
	function listado_solicitudes_revision_examenes(){
		...
		$this->load->model('wood/Solicitud_revision_examenes_previos_model');
		$this->load->model('wood/Usuarios_model');
		$this->load->model('wood/ciudades_model');
		$this->load->model('wood/Cargos_model');
		$this->load->model('wood/Empresa_planta_model');
		...
		$this->load->model('wood/Sre_evaluacion_req_agenda_model');
		...
		$base = array(
			...
			'side_bar' => true,
			'lugar' => array(array('url' => 'usuarios/home', 'txt' => 'Inicio'), array('url'=>'wood/trabajadores/buscar_js','txt'=>'Listado Trabajadores' ), array('url'=>'','txt'=>'Listado Trabajadores Revision Examenes Pendientes' ) ),
			'menu' => $this->menu,
		...
		$base['cuerpo'] = $this->load->view('wood/trabajadores/listado_solicitudes_revision_examen',$pagina,TRUE);
		$this->load->view('layout2.0/layout',$base);
	}
	...
	function solicitudes_revision_examenes(){
		...
		$base = array(
			...
			'side_bar' => true,
			'lugar' => array(array('url' => 'usuarios/home', 'txt' => 'Inicio'), array('url'=>'wood/trabajadores/buscar_js','txt'=>'Listado Trabajadores' ), array('url'=>'','txt'=>'Listado Trabajadores Revision Examenes' ) ),
			'menu' => $this->menu,
		...
		$pagina['lista_requerimientos'] = $this->Requerimientos_model->r_listar_activos();
		$pagina['r_cargos'] = $this->Cargos_model->r_listar();
		$base['cuerpo'] = $this->load->view('wood/trabajadores/listado_envio_solicitud_revision_examen',$pagina,TRUE);
		$this->load->view('layout2.0/layout',$base);
	}
	...
	function trabajador_wood($id, $id_evaluacion = FALSE){
		//$this->load->model("wood/Tipoarchivos_model");
		$this->load->model("wood/Archivos_trab_model");
		$this->load->model("wood/estado_civil_model");
		$this->load->model("wood/Usuarios_model");
		$this->load->model("wood/regiones_model");
		//$this->load->model("wood/Provincia_model");
		$this->load->model("wood/ciudades_model");
		//$this->load->model("wood/Bancos_model");
		$this->load->model("wood/Afp_model");
		$this->load->model("wood/Salud_model");
		//$this->load->model("wood/Nivelestudios_model");
		$this->load->model("wood/Usu_parentesco_model");
		$this->load->model("wood/Profesiones_model");
		//$this->load->model("wood/Especialidadtrabajador_model");
		//$this->load->model("wood/Experiencia_model");
		$this->load->model("wood/Evaluaciones_model");
		//Masso
		//$this->load->model('wood/Evaluacionestipo_model');
		//$this->load->model('wood/Evaluacionesbaterias_model');
		...
		$base = array(
			...
			'subtitulo' => 'Unidad de Negocio: Celulosa Nueva Aldea',
			'side_bar' => true,
			'lugar' => array(array('url' => 'usuarios/home', 'txt' => 'Inicio'), array('url'=>'wood/trabajadores/buscar_js','txt'=>'Trabajadores EST' ), array('url'=>'','txt'=>'Datos Trabajadores' )),
		...
		$base['cuerpo'] = $this->load->view('trabajadores/trabajador_wood',$pagina,TRUE);
		$this->load->view('layout2.0/layout',$base);
	}
	...
	function modal_editar_sre_eval_req($id_registro, $estado = FALSE, $idExamen = false){
		...
		$this->load->model("wood/Cargos_model");
		$this->load->model("wood/Solicitud_revision_examenes_model");
		$this->load->model("wood/Examenes_psicologicos_model");
		...
		if ($idExamen) {// #08-01-2018 si vengo desde el perfil psicologo consultar sobre las fecha de vigencia del examen
			$this->load->model('wood/Usuarios_model');
			$this->load->model('wood/Examenes_psicologicos_model');
			$this->load->model('wood/Examenes_psicologicos_archivos_model');
			$this->load->model('wood/usuarios/Usuarios_general_model');
			$this->load->model('wood/Empresa_planta_model');
			$this->load->model('wood/Cargos_model');
			$this->load->model('wood/Especialidadtrabajador_model');
		...
		$this->load->view('wood/trabajadores/modal_editar_sre_evaluacion_req', $pagina);
	}
	...
	function modal_agendar_examen($id_solicitud, $tipo_examen, $estado = FALSE){
		...
		$this->load->view('wood/trabajadores/modal_agendar_solicitud_examen',$pagina);
	}
	...
	function ingresar_agenda_de_examen(){
		...
		redirect('wood/trabajadores/listado_solicitudes_revision_examenes', 'refresh');
	}
	...
	function solicitudes_revision_examenes_completas(){
		...
		$base = array(
			...
			'subtitulo' => 'Unidad de Negocio: WOOD',
			'side_bar' => true,
			'lugar' => array(array('url' => 'usuarios/home', 'txt' => 'Inicio'), array('url'=>'wood/trabajadores/buscar_js','txt'=>'Listado Trabajadores' ), array('url'=>'','txt'=>'Listado Trabajadores Revision Examenes Completas' ) ),
			'menu' => $this->menu,
			'js' => array('js/confirm.js','plugins/DataTables/media/js/jquery.dataTables.min.js','plugins/DataTables/FixedColumns/js/dataTables.fixedColumns.min.js','js/lista_req.js'),
			'css' => array('plugins/DataTables/media/css/jquery.dataTables.min.css','plugins/DataTables/media/css/dataTables.bootstrap.min.css','plugins/DataTables/FixedColumns/css/fixedColumns.dataTables.css'),
		);
		...
		$pagina['lista_requerimientos'] = $this->Requerimientos_model->r_listar_activos();
		$pagina['r_cargos'] = $this->Cargos_model->r_listar();
		$base['cuerpo'] = $this->load->view('wood/trabajadores/listado_solicitudes_revision_examen_visualizacion',$pagina,TRUE);
		$this->load->view('layout2.0/layout',$base);
	}
	...
	function eliminar_examen($id_usr, $id_evalua=FALSE){
		...
		redirect('wood/trabajadores/detalle/'.$id_usr.'#datos-personales', 'refresh');	
	}
	function guardar_archivo($id){
		if($_FILES['documento']['error'] == 0){
			...
			if($_POST['select_archivo'] == 13){
				...
				foreach ($la as $l) {
				 	if( $l->id_tipoarchivo == 13)
				 		redirect('wood/trabajadores/detalle/'.$id.'/archivo_repetido#datos-extras', 'refresh');
				}
			}
			...
			if($salida == 1)
				redirect('wood/trabajadores/detalle/'.$id.'/archivo_error0#datos-extras', 'refresh');
			elseif($salida==2)
				redirect('wood/trabajadores/detalle/'.$id.'/archivo_error1#datos-extras', 'refresh');
			else{
				$data = array(
					'id_usuarios' => $id,
					'nombre' => $nb_archivo,
				...
				redirect('wood/trabajadores/detalle/'.$id.'/archivo_exito#datos-extras', 'refresh');
			}
		}
		else redirect('wood/trabajadores/detalle/'.$id.'/archivo_vacio#datos-extras', 'refresh');
	}
	function rut_existe(){
		$this->load->model("wood/Usuarios_model");
		if ( $_POST['rut'] ){
