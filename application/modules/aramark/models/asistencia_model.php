...
class Asistencia_model extends CI_Model {
	...
	function get_trabajadores_activo_asistencia(){
		$this->aramark->select('*');
		$this->aramark->from('usuarios');
		$this->aramark->where('estadoAsistencia', 1);
		$this->aramark->order_by('id','asc');
		$query = $this->aramark->get();
		return $query->result();
	}
	...
	function getAsistenciaPersona($id, $fecha){
		...
		$this->aramark->where("id_usuario",$id);
		$this->aramark->where("fecha",$fecha);
		$query = $this->aramark->get();
		return $query->row();
	}
	#27-07-2018
	...
	function cambiarEstadoAsistencia($id_usuario, $data){
		$this->aramark->where('id', $id_usuario);
		$this->aramark->update('usuarios', $data); 
	}
	...
	function actualizarAsistenciaTrabajador($idTrabajador, $data, $fechaSeleccionada){
		$this->aramark->where('id_usuario', $idTrabajador);
		$this->aramark->where('fecha', $fechaSeleccionada);
		$this->aramark->update('asistencia', $data); 
	}
	#30-07-2018
	function verificarSiYaGuardadoBonoDelMes($fecha){
		$this->aramark->select('*');
		$this->aramark->from('bonos_anticipos');
		$this->aramark->where("fecha",$fecha);
		$query = $this->aramark->get();
		if ($query->num_rows() > 0){
			return true;
	...
	function guardarBonoTrabajador($data){
		$this->aramark->insert('bonos_anticipos',$data);
	}
	...
	function actualizarBonoTrabajador($idTrabajador, $data, $fechaSeleccionada){
		$this->aramark->where('id_usuario', $idTrabajador);
		$this->aramark->where('fecha', $fechaSeleccionada);
		$this->aramark->update('bonos_anticipos', $data); 
	}
}
?>