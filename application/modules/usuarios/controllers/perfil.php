...
class Perfil extends CI_Controller{
	...
	function crear($msg = FALSE){
		...
		$pagina['lista_provincia'] = $this->Provincia_model->listar();
		$pagina['lista_ciudad'] = $this->Ciudad_model->listar();
		...
		$base['cuerpo'] = $this->load->view('perfil/creando_trabajador',$pagina,TRUE);
		$this->load->view('layout2.0/layout',$base);
	}
	...
	function guardar_trabajador(){
			...
			if(empty($_POST['nombres']) || empty($_POST['paterno']) || empty($_POST['rut']) ){
		 		redirect('usuarios/crear/error_vacio', 'refresh');
		 	}
			...
			//var_dump($id_usr);return false;
		//INICIO API
	    $url = 'http://54.232.203.239/wsIntegra/insertPersonal';
		$ch = curl_init($url);
		$data = array(
			    'id_personal_SGO' =>$id_usr,
				'funcion' =>'',
				'rut' =>trim($_POST['rut']),
				'nombre' =>trim($_POST['nombres']),
				'apellido_paterno' =>trim($_POST['paterno']),
				'apellido_materno' =>trim($_POST['materno']),
				'telefono_empresa' =>trim($_POST['fono1']),
				'telefono_personal' =>trim($_POST['fono1']),
				'direccion_personal' =>$_POST['direccion'],
				'mail_empresa' =>trim($_POST['email']),
				'mail_personal' =>trim($_POST['email']),
				...
				'numero_cuenta'=>$_POST['n_cuenta'],
				'id_tipo_cuenta'=>1,//Debuguear
				'id_banco'=>$_POST['select_bancos'],
				//'id_planta'=>
				//'id_cliente'=>1
			);//993308819
        ...
        //FIN API
			...
			//if ( ($_POST['tipo_usuario'] == 3) && ( $_POST['cargo'] == 2) ){
				$this->load->model("Usuarios2_model");
		   		...
		   		$this->session->set_userdata('trabajador_creado', true);
	...
	function prueba_envio_json(){
		//API URL
		//$url = 'http://localhost/sgo2/api/updateTrabajador';
	...
	function guardar_personales(){
		...
		if(empty($_POST['select_region']) || empty($_POST['select_provincia']) || empty($_POST['select_ciudad']) || empty($_POST['select_civil']) || empty($_POST['select_nacionalidad']) || empty($_POST['nombres']) || empty($_POST['paterno']))
		...
		if($this->session->userdata('tipo_usuario') == 1 or $this->session->userdata('id') == 39 or $this->session->userdata('id') == 81){
			$data_rut = array(
		...
		$todos_los_datos = array_merge($data_rut, $data);
		$this->Usuarios_model->editar($id_usuario,$todos_los_datos);
		//if ( $_FILES['imagen'] )
