...
class Requerimientos_model extends CI_Model{
	...
	function ingresar($data){
		$this->log_serv->insert('r_requerimiento',$data);
		return $this->log_serv->insert_id();
	}

	function r_listar_order_estado(){
		$this->log_serv->order_by("estado", "desc"); 
		$query = $this->log_serv->get('r_requerimiento');
		return $query->result();
	}

	function actualizar_desactivo_requerimiento($datos){
		$this->log_serv->update('r_requerimiento', $datos);
	}
	
	function actualizar($id_req, $data){
		$this->log_serv->where('id', $id_req);
		$this->log_serv->update('r_requerimiento', $data);
	}

	function eliminar($id){
		$this->log_serv->delete('r_requerimiento', array('id' => $id)); 
	}
	...
	function get_req_planta($id){
		...
		$this->log_serv->from('r_requerimiento');
		$this->log_serv->join('empresa_planta', 'empresa_planta.id = r_requerimiento.planta_id');
		$this->log_serv->where("r_requerimiento.id",$id);
