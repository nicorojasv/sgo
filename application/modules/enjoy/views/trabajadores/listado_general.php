...
<div class="panel panel-white">
	...
	<div class="panel-body">
		<div class="row" >
            ...
            <form action="<?php echo base_url() ?>enjoy/trabajadores/exportar_excel_contratos_y_anexos" method="post" target="_blank" id="FormularioExportacion">
              <input title="EXPORTAR DATOS A ARCHIVO EXCEL" type="image" src="<?php echo base_url() ?>extras/imagenes/Excel-Export.jpg" class="botonExcelEnjoyTrabajadores " value="Exportar a Excel">
		...
		<table id="ejemplo" class="table table-striped table-bordered" style="width:100%">
			...
			<tbody>
				...
				<tr>
					<!--<td><?php echo $i ?></td>-->
					<td style="width: 10%"><?php echo $row->rut_usuario ?></td>
					<td style="width: 30%"><?php echo $row->nombres." ". $row->paterno." ". $row->materno ?></td>
					<td style="width: 10%"><?php echo $row->fono ?></td>
					...
					<td style="width: 30%;overflow: auto;" id="<?php echo $row->id_usuario ?>">
						<div class="col-md-10" >
							<textarea  type="text" style="display: block; width: 100%;"  class="edit" name="observacion"    onblur="myFunction(<?php echo $row->id_usuario ?>, this.value)"><?php echo isset($row->observacion)?$row->observacion:'';?></textarea>
						</div>
						<div class="col-md-2">
							<?php 
							...
							?>
								<!--<label style="font-size:8px; text-align:center; color:#807b7b ">Editado por: <?php echo $row->usuario_observacion ?> </label>-->
								<a class="EliminarObservacionEnjoy" title="Eliminar Observacion" data-id="<?php echo $row->id_usuario ?>" style="cursor:pointer;" data-delete="<?php echo $row->id_usuario ?>" ><i class="fa fa-times " style="font-size: 10px; color: red;" aria-hidden="true"></i></a>
							<?php		
								}
							?>
...
<script type="text/javascript">
  function myFunction(id,value) {
                  var datos= value;
                    ...
                    $.ajax({
                      type:"post",
                      url: base_url+"enjoy/trabajadores/guardarObservacion/"+id,
...
$(document).ready(function() {
    $('.EliminarObservacionEnjoy').click(function(){
    	var idUsuario = $(this).attr('data-id');
    	var idBorrar = $(this).attr('data-delete');
		alertify.confirm('Esta seguro de eliminar Observacion', '', function(){ 
	    		$.ajax({
	            type: "POST",
	            url: base_url+"enjoy/trabajadores/eliminar_observacion/"+idUsuario,
	            data: idUsuario,
	            dataType: "json",
		            success: function(data) {  
		            	if (data == 1) {
