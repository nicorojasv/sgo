...
class Requerimientos_model extends CI_Model{
	...
	function ingresar($data){
		$this->enjoy->insert('r_requerimiento',$data);
		return $this->enjoy->insert_id();
	}

	function r_listar_order_estado(){
		$this->enjoy->order_by("estado", "desc"); 
		$query = $this->enjoy->get('r_requerimiento');
		return $query->result();
	}

	function actualizar_desactivo_requerimiento($datos){
		$this->enjoy->update('r_requerimiento', $datos);
	}
	
	function actualizar($id_req, $data){
		$this->enjoy->where('id', $id_req);
		$this->enjoy->update('r_requerimiento', $data);
	}

	function eliminar($id){
		$this->enjoy->delete('r_requerimiento', array('id' => $id)); 
	}
	...
	function r_listar_order_estado_fecha($fecha_inicio, $fecha_termino){
		$this->enjoy->select("*"); 
		$this->enjoy->from("r_requerimiento"); 	
		$this->enjoy->order_by("id", "desc"); 
		//$this->enjoy->limit(10); 
		$this->enjoy->where('f_inicio BETWEEN "'.$fecha_inicio. '" and "'. $fecha_termino.'"');
	...
	function get_req_planta($id){
		$this->enjoy->select("*");
		$this->enjoy->from('r_requerimiento');
		$this->enjoy->join('empresa_planta', 'empresa_planta.id = r_requerimiento.planta_id');
		$this->enjoy->where("r_requerimiento.id",$id);
		$query = $this->enjoy->get();
		return $query->row();
	}
	...
	function getRequerimientoPuesto($id){
		$this->enjoy->select('rr.id, 
						  ...
						  ');
		$this->enjoy->from('r_requerimiento rr');
		$this->enjoy->join('empresa','empresa.id = rr.empresa_id','inner');
		$this->enjoy->join('empresa_planta','empresa_planta.id = rr.planta_id','inner');
		$this->enjoy->join('ciudades','ciudades.id = empresa_planta.id_ciudades','inner');
		$this->enjoy->where('rr.id',$id);
		$query = $this->enjoy->get();
		return $query->row();
	}
