...
class Requerimiento_area_cargo_model extends CI_Model {
	...
	function getReqContratoPuestoDisposicion($id){
		$this->enjoy->select('r_requerimiento_area_cargo.id as idAreaCargo, r_requerimiento_area_cargo.cantidad as cantidadTrabajadores, r_cargos.nombre as nombreCargo, r_areas.nombre as nombreArea, r_requerimiento_area_cargo.valor_aprox as valor');
		$this->enjoy->from('r_requerimiento_area_cargo ');
		$this->enjoy->join('r_cargos ','r_cargos.id = r_requerimiento_area_cargo.cargos_id','inner');
		$this->enjoy->join('r_areas ','r_areas.id = r_requerimiento_area_cargo.areas_id','inner');
		$this->enjoy->where('requerimiento_id',$id);
