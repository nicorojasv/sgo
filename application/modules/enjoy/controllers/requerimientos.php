...
class Requerimientos extends CI_Controller {
	...
	function index(){
		redirect('enjoy/requerimientos/listado', 'refresh');
	}
	...
	function agregar(){
		$base = array(
			...
			'subtitulo' => '',
			'side_bar' => true,
			'lugar' => array(array('url' => 'usuarios/home', 'txt' => 'Inicio'), array('url'=>'enjoy/requerimientos/listado','txt' => 'Listado Requerimientos'), array('url'=>'','txt'=>'Crear Requerimiento' )),
		...
		$pagina['soloParaHoraFecha']= false;
	...
	function asignacion($id){
		...
		$base = array(
			...
			'lugar' => array(array('url' => 'usuarios/home', 'txt' => 'Inicio'), array('url'=>'enjoy/requerimientos/listado','txt' => 'Listado Requerimientos'), array('url'=>'','txt'=>'Asignar Areas - Cargos Requerimiento' )),
			'menu' => $this->menu
		);
	...
	function guardar_area_cargo_req(){
		...
		redirect('enjoy/requerimientos/asignacion/'.$id_req.'', 'refresh');
	}
	...
	function listado($fecha = false){
		$base = array(
			...
			'menu' => $this->menu,
			'js' => array('js/confirm.js','plugins/DataTables/media/js/jquery.dataTables.min.js','plugins/DataTables/FixedColumns/js/dataTables.fixedColumns.min.js','js/lista_req.js','js/exportarExcelEnjoy.js'),
			'css' => array('plugins/DataTables/media/css/jquery.dataTables.min.css','plugins/DataTables/media/css/dataTables.bootstrap.min.css','plugins/DataTables/FixedColumns/css/fixedColumns.dataTables.css'),
		);
...
BEGIN
    ...
    DECLARE EXIT HANDLER FOR SQLWARNING
  	...
  	START TRANSACTION;
  	SET @nombre = id;
		SELECT  @id:=p.id_oferta , @idEm:= p.id_Empresa, @idRe:=p.id_Region, @idCi:=p.id_Ciudad, @idTipCon:=p.id_TipoContrato, @idJor:=p.id_JornadaLaboral, @idAre:=p.id_AreaTrabajo, @idExp:=p.id_AnosExperiencia, @idEst:=p.id_Estudio, @idDom:=p.id_DominioComputacion,@titu:= p.tituloOferta, @desOf:=p.descripcionOferta, @vac:=p.vacantesOferta, @sueldo:=p.sueldoOferta, @mostrar:=p.mostrarSueldoOferta, @reqMin:=p.requisitosMinimosOferta, @licen:=p.todasLicenciasOferta, @cant:=p.cantVisitas, @dest:=p.destacar FROM proyecto2.oferta p WHERE p.id_Oferta= @nombre;
		INSERT INTO proyecto.oferta (id_OfertaEmpresa, id_Empresa, id_Region,id_Ciudad ,id_TipoContrato ,id_JornadaLaboral ,id_AreaTrabajo, id_AnosExperiencia, id_Estudio, id_DominioComputacion, tituloOferta, descripcionOferta, vacantesOferta, sueldoOferta,mostrarSueldoOferta ,requisitosMinimosOferta ,todasLicenciasOferta ,cantVisitas ,destacar ) 
		VALUES(@nombre,@idEm,@idRe,@idCi,@idTipCon,@idJor,@idAre,@idExp,@idEst,@idDom,@titu,@desOf,@vac,@sueldo,@mostrar,@reqMin,@licen,@cant,@dest);
		INSERT INTO afp VALUES (1,'wena');
...
END*/
	function usuarios_requerimiento($id_area_cargo, $id_usuario = FALSE){
		...
		$this->load->model("enjoy/Requerimiento_area_cargo_model");
		$this->load->model("enjoy/Requerimientos_model");
		$this->load->model('enjoy/Archivos_trab_model');
		$this->load->model('enjoy/Empresas_model');
		...
		$base = array(
			...
			'subtitulo' => '',
			'side_bar' => false,
			'lugar' => array(array('url' => 'usuarios/home', 'txt' => 'Inicio'), array('url'=>'enjoy/requerimientos/listado','txt' => 'Listado Requerimientos'), array('url'=>'enjoy/requerimientos/asignacion/'.(isset($r_area_cargo->requerimiento_id)?$r_area_cargo->requerimiento_id:'0').'','txt' => 'Area-Cargo Requerimiento'), array('url'=>'','txt'=>'Listado de usuarios' )),
			'menu' => $this->menu,
	...
	function usuarios_requerimientos_listado($id_area_cargo){
		...
		$base = array(
			...
			'subtitulo' => '',
			'side_bar' => true,
			'lugar' => array(array('url' => 'usuarios/home', 'txt' => 'Inicio'), array('url'=>'enjoy/requerimientos/usuarios_requerimiento/'.$id_area_cargo."", 'txt'=>'Volver a la Area - Cargo del Requerimiento'), array('url'=>'','txt'=>"Listado Trabajadores requerimiento ".$this->Requerimientos_model->get($id_req)->nombre."" )),
			'menu' => $this->menu,
	...
	function agregar_usuarios_requerimiento(){
		...
		redirect('enjoy/requerimientos/usuarios_requerimiento/'.$id_area_cargo, 'refresh');
	}
	...
	function contratos_req_trabajador($id_usuario,$id_asc_area_req,$id_area_cargo_req = FALSE){
		...
		$this->load->model("enjoy/Tipo_gratificacion_model");
	...
	function eliminar_contratos_req_trabajador($id_usuario, $id_asc_area_req, $id_area_cargo_req, $id_req_usu_arch){
		...
		redirect('enjoy/requerimientos/contratos_req_trabajador/'.$id_usuario.'/'.$id_asc_area_req.'/'.$id_area_cargo_req.'', 'refresh');
	}
	...
	function guardar_nuevo_contrato_anexo_doc_contractual($usuario,$tipo,$asc_area){
		...
		if(empty($_POST['ano_fp']) || empty($_POST['mes_fp']) || empty($_POST['dia_fp']) )
			$fecha_pago = '0000-00-00';
		else
			$fecha_pago = $_POST['ano_fp'].'-'.$_POST['mes_fp'].'-'.$_POST['dia_fp'];
		...
		redirect('enjoy/requerimientos/contratos_req_trabajador/'.$usuario.'/'.$asc_area.'/'.$id_area_cargo.'', 'refresh');
	}
	...
	function actualizar_contrato_anexo_doc_contractual($id_usu_arch,$id_area_cargo){
		...
		if (isset($_POST['actualizar'])){
			...
			$sueldo_liquido_37mil = isset($get_afp->sueldo_liquido_37mil)?$get_afp->sueldo_liquido_37mil:0;
			$sueldo_liquido_40mil = isset($get_afp->sueldo_liquido_40mil)?$get_afp->sueldo_liquido_40mil:0;
			$sueldo_liquido_42mil = isset($get_afp->sueldo_liquido_42mil)?$get_afp->sueldo_liquido_42mil:0;
			...
			}elseif($renta_imponible == 37000){
				$renta_imponible = round($sueldo_liquido_37mil / 30);
			}elseif($renta_imponible == 40000){
				$renta_imponible = round($sueldo_liquido_40mil / 30);
			}elseif($renta_imponible == 42000){
				$renta_imponible = round($sueldo_liquido_42mil / 30);
	...
	function eliminar_usuarios_req($id,$area_cargo){
		...
		redirect('enjoy/requerimientos/usuarios_requerimiento/'.$area_cargo, 'refresh');
	}
	...
	function eliminar($id){
		...
		redirect(base_url().'enjoy/requerimientos/listado', 'refresh');
	}
...
//				$valorTotalRedondeado[$i] = round($valorTotal[$i]);
			...
			if ($requerimientos->idEmpresa == 2) {
				$yempresa  = 'e '.titleCase($requerimientos->razonSocial);
			}else{
				$yempresa  = 'y '.titleCase($requerimientos->razonSocial);
			}
			//$subtotal = ($sueldoBaseMasGratificacion[$i]/30)*$totalDiasRequerimiento;
			$template_formato = base_url()."extras/contratos/formatos_contrato_disposicion/formatoContratoPuestaDisposicionEnjoy.docx";
			$templateWord = new TemplateProcessor($template_formato);
			$templateWord->setValue('fechaSolicitudRequerimiento', $fechaSolicitudFormateada);
			$templateWord->setValue('letraCausal',$requerimientos->letraCausal);
			$templateWord->setValue('nombreEmpresa',titleCase($requerimientos->razonSocial));
			$templateWord->setValue('ynombreEmpresa',$yempresa);
			$templateWord->setValue('rolEmpresa',$requerimientos->rut);
			$templateWord->setValue('descripcionLetraCausal',$descripcionLetraCausal);
			$templateWord->setValue('seRequiere',$seRequiere);
			$templateWord->setValue('motivoRequerimiento',htmlspecialchars($requerimientos->motivo));
			$templateWord->setValue('totalDiasRequerimiento',$totalDiasRequerimiento);
			$templateWord->setValue('art183',$art183);
			$templateWord->setValue('remuneracionNumero',$ValorReTotal);
			...
			$templateWord->setValue('fehcaTerminoRequerimiento',$fechaFinFormateada);
			// Guardamos el documento
