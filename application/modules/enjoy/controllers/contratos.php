	...
	function solicitudes_pendientes(){
		...
		if($trabajadores != FALSE){
			foreach($trabajadores as $rm){
				...
				$aux->nombre_centro_costo = isset($get_centro_costo->razon_social)?ucwords(strtolower($get_centro_costo->razon_social)):'';
				...
				if($renta_imponible == 15000){
					...
					$sueldoMensualDeAcuerdoAsuAFP = $sueldo_liquido_37mil;
					...
					$sueldoMensualDeAcuerdoAsuAFP = $sueldo_liquido_42mil;
	...
	function generar_contrato_anexo_doc_contractual(){
		...
		if (isset($_POST['dar_de_baja'])) { //para dar de baja un contrato
			...
			$nombre_trabajador = $nombre." ".$aP." ".$aM;
				...
				$config['smtp_port']    = '2552';
				$this->email->initialize($config);
				$destinatarios_cc = array('contratos@empresasintegra.cl','jsilva@empresasintegra.cl','supervisorenjoy@empresasintegra.cl');
			    $this->email->from('informaciones@empresasintegra.cl', 'Solicitud Contrato SGO - ENJOY');
			...
			/*$this->load->library('email');
			    ...
			    $this->email->to('sastudillo@empresasintegra.cl');
				$this->email->cc($destinatarios_cc);
			    $this->email->subject("Baja de Contrato Enjoy Trabajador: ".$nombre_trabajador." RECHAZADA ");
	...
	function solicitud_bajar_contrato($id_usu_arch){
		if (empty($_POST['value']) || $_POST['value']=='Seleccione Motivo de baja') {
			...
			$hoy = date('Y-m-d');
			    ...
			    $this->email->subject("Baja de Contrato Enjoy Trabajador: ".$nombre_trabajador." fecha: ".$hoy." PENDIENTE ");
