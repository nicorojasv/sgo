...
class Contratos extends CI_Controller{
	...
	function index(){
		redirect('terramar/contratos/solicitudes_pendientes', 'refresh');
	}
	...
	function solicitudes_pendientes($get_id_planta = FALSE){
       ...
       //return false;
		...
		$base['cuerpo'] = $this->load->view('terramar/contratos/listado_solicitudes_pendientes',$pagina,TRUE);
	...
	function aprobar_contrato_anexo_doc_general($id_req_usu_arch){
		...
		if($get_usu_archivo->jornada == "1"){
			if ($id_centro_costo==2 || $id_centro_costo == 4 || $id_centro_costo==5) {
				$template_formato = base_url()."extras/contratos/formatos_contratos_terramar/2_contrato_sin_pacto_heNuevo.docx";
			}else{
				$template_formato = base_url()."extras/contratos/formatos_contratos_terramar/2_contrato_sin_pacto_he.docx";
			}
		}else{
			if ($id_centro_costo==2 || $id_centro_costo == 4 || $id_centro_costo==5) {
				$template_formato = base_url()."extras/contratos/formatos_contratos_terramar/2_contrato_sin_pacto_heNuevo.docx";
			}else{
				$template_formato = base_url()."extras/contratos/formatos_contratos_terramar/1_contrato_con_pacto_he.docx";
			}
		}
	...
	//	return false;
		...
		$pagina['horarios_planta'] = $this->Descripcion_horarios_model->listar_planta($id_planta);
		$pagina['datos_usu_arch']= $this->Requerimiento_Usuario_Archivo_model->get_result($id_usu_arch);
		$this->load->view('terramar/contratos/modal_administrar_contrato_anexo_doc_contractuales', $pagina);
	}
	...
	function generar_contrato_anexo_doc_contractual(){
		...
		if (isset($_POST['dar_de_baja'])) { //para dar de baja un contrato
			...
			/*	$this->load->library('email');
			    ...
			    $this->email->send();*/
			redirect('terramar/contratos/solicitudes_pendientes_baja','refresh');
			return false;
			...
			if($id_planta == 2)
				$template_formato = base_url()."extras/contratos/formatos_contratos_terramar/3_contrato_parte_2_con_4_documentos_celulosa_terramar.docx";
			elseif($id_planta == 3)
				$template_formato = base_url()."extras/contratos/formatos_contratos_terramar/3_contrato_parte_2_con_4_documentos_celulosa_nueva_aldea.docx";
			elseif($id_planta == 5)
				$template_formato = base_url()."extras/contratos/formatos_contratos_terramar/3_contrato_parte_2_con_4_documentos_celulosa_valdivia.docx";
			elseif($id_planta == 30)
				$template_formato = base_url()."extras/contratos/formatos_contratos_terramar/3_contrato_parte_2_con_4_documentos_forestal_chillan.docx";
			elseif($id_planta == 20 or $id_planta == 21 or $id_planta == 34 or $id_planta == 35 or $id_planta == 36 or $id_planta == 37 or $id_planta == 38 or $id_planta == 39 or $id_planta == 40 )
				$template_formato = base_url()."extras/contratos/formatos_contratos_terramar/4_contrato_parte_2_con_2_documentos.docx";
			elseif($id_planta == 6 or $id_planta == 8 or $id_planta == 9 or $id_planta == 10 or $id_planta == 11 or $id_planta == 15 or $id_planta == 16 or $id_planta == 17 or $id_planta == 19 or $id_planta == 33)
				$template_formato = base_url()."extras/contratos/formatos_contratos_terramar/3_contrato_parte_2_con_4_documentos_maderas.docx";
			else
				$template_formato = base_url()."extras/contratos/formatos_contratos_terramar/3_contrato_parte_2_con_4_documentos_celulosa_y_gic_y_transversal.docx";
	...
	function solicitud_bajar_contrato($id_usu_arch){
		if($id_usu_arch != FALSE ){
			...
			$hoy = date('Y-m-d');
			    ...
			    $this->email->subject("Baja de Contrato terramar Trabajador: ".$nombre_trabajador." fecha: ".$hoy." PENDIENTE ");
	...
	function solicitudes_completas_baja(){
		...
		$pagina['listado'] = $listado;
		$pagina['listado_plantas'] = $this->Empresa_planta_model->listar();
		$base['cuerpo'] = $this->load->view('terramar/contratos/listado_solicitudes_completas',$pagina,TRUE);
		$this->load->view('layout2.0/layout',$base);
	}
	...
	*/
	...
	function aprobar_anexo($id){
		...
		$carpeta = "extras/anexos_masivos_terramar/".$requerimientou->requerimiento_id;
		if (!file_exists($carpeta)) {
		    mkdir($carpeta, 0777, true);
		}
		$templateWord->saveAs("extras/anexos_masivos_terramar/".$requerimientou->requerimiento_id."/".$nombre_documento);
		ob_clean();
		$get_url = "extras/anexos_masivos_terramar/".$requerimientou->requerimiento_id."/".$nombre_documento;
		...
		$email_solicitante = isset($get_solicitante->email)?$get_solicitante->email:'';
				...
				$config['smtp_port']    = '2552';
				$this->email->initialize($config);
			    $this->email->from('informaciones@empresasintegra.cl', 'Solicitud Anexo de Contrato SGO - terramar');
			    $this->email->to($email_solicitante);
			    //$this->email->to('nrojas@empresasintegra.cl');
				$this->email->cc('contratos@empresasintegra.cl');
		...
		redirect('terramar/contratos/solicitudes_pendientes_anexo/', 'refresh');
	}
