...
class Contratos extends CI_Controller{
	...
	function index(){
		redirect('sanatorio/contratos/solicitudes_pendientes', 'refresh');
	}
	...
	function solicitudes_pendientes($get_id_planta = FALSE){
       ...
       //return false;
		...
		$base['cuerpo'] = $this->load->view('sanatorio/contratos/listado_solicitudes_pendientes',$pagina,TRUE);
	...
	//	return false;
		...
		$pagina['horarios_planta'] = $this->Descripcion_horarios_model->listar_planta($id_planta);
		$pagina['datos_usu_arch']= $this->Requerimiento_Usuario_Archivo_model->get_result($id_usu_arch);
		$this->load->view('sanatorio/contratos/modal_administrar_contrato_anexo_doc_contractuales', $pagina);
	}
	...
	function generar_contrato_anexo_doc_contractual(){
		...
		if (isset($_POST['dar_de_baja'])) { //para dar de baja un contrato
			...
			/*	$this->load->library('email');
			    ...
			    $this->email->send();*/
			redirect('sanatorio/contratos/solicitudes_pendientes_baja','refresh');
			return false;
			...
			foreach($_POST['solicitudes'] as $c=>$valores){
				...
				if($get_usu_archivo->jornada == "1"){
					if ($id_centro_costo==2 || $id_centro_costo == 4 || $id_centro_costo==5) {
						$template_formato = base_url()."extras/contratos/formatos_contratos_sanatorio/2_contrato_sin_pacto_heNuevo.docx";
					}else{
						$template_formato = base_url()."extras/contratos/formatos_contratos_sanatorio/2_contrato_sin_pacto_he.docx";
					}
				}else{
		...
		if($id_usu_arch != FALSE ){
			...
			$hoy = date('Y-m-d');
			   ...
			   // $this->email->to('contratos@empresasintegra.cl');
			    ...
			    $this->email->subject("Baja de Contrato sanatorio Trabajador: ".$nombre_trabajador." fecha: ".$hoy." PENDIENTE ");
	...
	function solicitudes_completas_baja(){
		...
		$pagina['listado'] = $listado;
		$pagina['listado_plantas'] = $this->Empresa_planta_model->listar();
		$base['cuerpo'] = $this->load->view('sanatorio/contratos/listado_solicitudes_completas',$pagina,TRUE);
		$this->load->view('layout2.0/layout',$base);
	}
	...
	*/
	...
	function aprobar_anexo($id){
		...
		$email_solicitante = isset($get_solicitante->email)?$get_solicitante->email:'';
				...
				$config['smtp_port']    = '2552';
				$this->email->initialize($config);
			    $this->email->from('informaciones@empresasintegra.cl', 'Solicitud Anexo de Contrato SGO - sanatorio');
			    $this->email->to($email_solicitante);
			    $this->email->to('nrojas@empresasintegra.cl');
				//$this->email->cc('contratos@empresasintegra.cl');
		...
		redirect('sanatorio/contratos/solicitudes_pendientes_anexo/', 'refresh');
	}
