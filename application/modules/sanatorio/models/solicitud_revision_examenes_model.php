...
class Solicitud_revision_examenes_model extends CI_Model{
	...
	function guardar($datos){
		$this->sanatorio->insert('solicitud_revision_examenes',$datos);
		return $this->sanatorio->insert_id();
	}

	function actualizar($id_registro, $datos){
		$this->sanatorio->where('id', $id_registro);
		$this->sanatorio->update('solicitud_revision_examenes', $datos); 
	}

	function get_result_en_proceso(){
		$this->sanatorio->select('*');
		$this->sanatorio->from('solicitud_revision_examenes');
...
//INSERT IGNORE INTO tabla_destino (campo1, campo2) SELECT tabla_origen.campo1, tabla_origen.campo2 FROM tabla_origen
	function get_result_solicitudes_usu($id_usu){
		$this->sanatorio->select('*');
		$this->sanatorio->from('solicitud_revision_examenes');
		$this->sanatorio->where('solicitante_id', $id_usu);
		$query = $this->sanatorio->get();
		return $query->result();
	}
...
*/
	...
	function get_usu_req($id_usu, $id_req, $id_asc){
		...
		$this->sanatorio->where('id_requerimiento', $id_req);
		$this->sanatorio->where('id_asc_trabajador', $id_asc);
		$query = $this->sanatorio->get();
		return $query->row();
	}
