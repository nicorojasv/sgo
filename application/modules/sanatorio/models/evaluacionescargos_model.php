...
class Evaluacionescargos_model extends CI_Model {
	...
	function ingresar($data){
		$this->sanatorio->insert('evaluaciones_cargos',$data); 
		return $this->sanatorio->insert_id();
	}

	function eliminar_eval($id){
		$this->sanatorio->delete('evaluaciones_cargos', array('id_evaluacion' => $id)); 
	}

	function get_eval($id){
		$this->sanatorio->where('id_evaluacion',$id);
		$query = $this->sanatorio->get('evaluaciones_cargos');
		return $query->result();
	}

	function get($cargo_id, $id_evaluacion = FALSE){
		$this->sanatorio->where('id_r_cargo',$cargo_id);
