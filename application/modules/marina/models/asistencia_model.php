...
class Asistencia_model extends CI_Model {
	...
	function get_trabajadores_activo_asistencia(){
		$this->marina->select('*');
		$this->marina->from('usuarios');
		$this->marina->where('estadoAsistencia', 1);
		$this->marina->order_by('id','asc');
		$query = $this->marina->get();
		return $query->result();
	}
	...
	function cambiarEstadoAsistencia($id_usuario, $data){
		$this->marina->where('id', $id_usuario);
		$this->marina->update('usuarios', $data); 
	}
	...
	function actualizarAsistenciaTrabajador($idTrabajador, $data, $fechaSeleccionada){
		$this->marina->where('id_usuario', $idTrabajador);
		$this->marina->where('fecha', $fechaSeleccionada);
		$this->marina->update('asistencia', $data); 
	}
	#30-07-2018
	function verificarSiYaGuardadoBonoDelMes($fecha){
		$this->marina->select('*');
		$this->marina->from('bonos_anticipos');
		$this->marina->where("fecha",$fecha);
		$query = $this->marina->get();
		if ($query->num_rows() > 0){
			return true;
	...
	function guardarBonoTrabajador($data){
		$this->marina->insert('bonos_anticipos',$data);
	}
	...
	function actualizarBonoTrabajador($idTrabajador, $data, $fechaSeleccionada){
		$this->marina->where('id_usuario', $idTrabajador);
		$this->marina->where('fecha', $fechaSeleccionada);
		$this->marina->update('bonos_anticipos', $data); 
	}
}
?>