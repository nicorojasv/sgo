...
class Requerimiento_area_cargo_model extends CI_Model {
	...
	function get($id){
		...
		$query = $this->marina->get('r_requerimiento_area_cargo');
		return $query->row();
	}
	...
	function get_requerimiento($id){
		$this->marina->where('requerimiento_id',$id);
		$query = $this->marina->get('r_requerimiento_area_cargo');
		return $query->result();
	}
	...
	function eliminar($id){
		$this->marina->delete('r_requerimiento_area_cargo', array('id' => $id)); 
	}

	function ingresar($data){
		$this->marina->insert('r_requerimiento_area_cargo',$data); 
		return $this->marina->insert_id();
	}

	function actualizar($data,$id){
		$this->marina->where('id', $id);
		$this->marina->update('r_requerimiento_area_cargo', $data); 
	}
		#29-11-2018

	function getReqContratoPuestoDisposicion($id){
		$this->marina->select('r_requerimiento_area_cargo.id as idAreaCargo, r_requerimiento_area_cargo.cantidad as cantidadTrabajadores, r_cargos.nombre as nombreCargo, r_areas.nombre as nombreArea, r_requerimiento_area_cargo.valor_aprox as valor');
		$this->marina->from('r_requerimiento_area_cargo ');
		$this->marina->join('r_cargos ','r_cargos.id = r_requerimiento_area_cargo.cargos_id','inner');
		$this->marina->join('r_areas ','r_areas.id = r_requerimiento_area_cargo.areas_id','inner');
		$this->marina->where('requerimiento_id',$id);
