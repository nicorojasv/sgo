...
class Requerimientos_model extends CI_Model{
	...
	function ingresar($data){
		$this->marina->insert('r_requerimiento',$data);
		return $this->marina->insert_id();
	}

	function r_listar_order_estado(){
		$this->marina->order_by("estado", "desc"); 
		$query = $this->marina->get('r_requerimiento');
		return $query->result();
	}

	function actualizar_desactivo_requerimiento($datos){
		$this->marina->update('r_requerimiento', $datos);
	}
	
	function actualizar($id_req, $data){
		$this->marina->where('id', $id_req);
		$this->marina->update('r_requerimiento', $data);
	}

	function eliminar($id){
		$this->marina->delete('r_requerimiento', array('id' => $id)); 
	}
	...
	function r_listar_order_estado_fecha($fecha_inicio, $fecha_termino){
		$this->marina->select("*"); 
		$this->marina->from("r_requerimiento"); 	
		$this->marina->order_by("id", "desc"); 
		//$this->marina->limit(10); 
		$this->marina->where('f_inicio BETWEEN "'.$fecha_inicio. '" and "'. $fecha_termino.'"');
	...
	function get_req_planta($id){
		$this->marina->select("*");
		$this->marina->from('r_requerimiento');
		$this->marina->join('empresa_planta', 'empresa_planta.id = r_requerimiento.planta_id');
		$this->marina->where("r_requerimiento.id",$id);
		$query = $this->marina->get();
		return $query->row();
	}
	...
	function getRequerimientoPuesto($id){
		$this->marina->select('rr.id, 
						  ...
						  ');
		$this->marina->from('r_requerimiento rr');
		$this->marina->join('empresa','empresa.id = rr.empresa_id','inner');
		$this->marina->join('empresa_planta','empresa_planta.id = rr.planta_id','inner');
		$this->marina->join('ciudades','ciudades.id = empresa_planta.id_ciudades','inner');
		$this->marina->where('rr.id',$id);
		$query = $this->marina->get();
		return $query->row();
	}
