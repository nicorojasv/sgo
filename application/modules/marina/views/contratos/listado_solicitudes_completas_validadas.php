      ...
      <label for="historico" style="cursor: pointer;" >Historico</label>
        ...
        <?php }?>
          <div class="col-md-4" align="center">
            <form action="<?php echo base_url() ?>marina/contratos/exportar_excel_contratos_y_anexos" method="post" target="_blank" id="FormularioExportacion">
          ...
          <table id="example1">
            ...
            <tbody>
              ...
              <tr id="<?php echo $i ?>">
                ...
                ?>
                <td>
                  <a data-toggle="modal" href="<?php echo base_url() ?>marina/contratos/modal_visualizar_contrato_anexo_doc_general/<?php echo $rm->id_req_usu_arch ?>" data-target="#ModalEditar" title="Visualizar mas detalles"><i class="fa fa-search" aria-hidden="true"></i></a>
                  <?php if($this->session->userdata('tipo_usuario') == 16 || $this->session->userdata('tipo_usuario') == 8 ){?>

                  <i class="fa fa-minus" style="color: #ccc" aria-hidden="true"></i>
                 <!-- <a href="<?php echo base_url() ?>marina/contratos/solicitud_bajar_contrato/<?php echo $rm->id_req_usu_arch ?>" title="Solicitar baja contrato" onClick="if(confirm('¿Esta seguro de dar de baja este contrato ?')) return true; else return false;"><i class="fa fa-sign-out" style="color: red" aria-hidden="true"></i></a>-->
        ...
        var idQuitar = $(this).attr('data-quitar');
          alertify.prompt('Baja de Contrato',"Contrato de "+nombre+".<br><br>Especifique motivo de solicitud de baja: <select id ='motivoBaja' onchange='remove(this.value);'><option>Seleccione Motivo de baja</option><option>Inasistencia con aviso</option><option>Inasistencia sin aviso</option><option>Cambio de Fecha</option><option>Cambio de Horario</option><option>Cambio de Renta</option><option>Cambio de Cargo</option><option>Modificación Motivo</option><option>Modificación Causal</option><option>Presentarse sin Uniforme</option><option>Cambio de Ceco y/o Area</option><option>Anulación requerimiento</option><option>Duplicidad</option><option>Otro</option></select>", "",
            function(evt, value ){// si presiona ok hacer esto
              $.ajax({
                  type: "POST",
                  url: base_url+"marina/contratos/solicitud_bajar_contrato/"+service,
